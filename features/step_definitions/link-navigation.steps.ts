...
Then('the event should be properly stopped to prevent double navigation', function() {
...
Then('both panes should be visible', function() {
  ...
  const visibleLeaves = leaves.filter(leaf => leaf.view?.containerEl?.offsetParent !== null);
...
Then('only Obsidian navigation should occur', function() {
  ...
  expect(openLinkTextSpy).toHaveBeenCalledWith('test-note.md', '', expect.any(Boolean));
});
