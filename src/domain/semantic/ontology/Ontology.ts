...
export class Ontology {
  ...
  addClass(definition: ClassDefinition): Result<void> {
    ...
    for (const superClass of definition.superClasses) {
      if (!this.classes.has(superClass.toString()) && 
    ...
    this.graph.add(new Triple(definition.iri, RDFS.label, Literal.string(definition.label)));
    
    if (definition.comment) {
      this.graph.add(new Triple(definition.iri, RDFS.comment, Literal.string(definition.comment)));
    }
  ...
  addProperty(definition: PropertyDefinition): Result<void> {
    ...
    for (const domainClass of definition.domain) {
      if (!this.classes.has(domainClass.toString()) && 
    ...
    // Add to RDF graph
    const propType = typeof definition.range === 'string' ? 
    ...
    if (definition.range instanceof IRI) {
      ...
      this.graph.add(new Triple(definition.iri, RDFS.range, this.dataTypeToIRI(definition.range)));
    }
  ...
  getSubclasses(classIri: IRI, direct: boolean = false): IRI[] {
    ...
    for (const classDef of this.classes.values()) {
      if (direct) {
        ...
        if (classDef.superClasses.some(s => s.toString() === classIri.toString())) {
  ...
  toTurtle(): string {
    ...
    // Properties
    for (const prop of this.properties.values()) {
      const propType = typeof prop.range === 'string' ? 
        'owl:DatatypeProperty' : 'owl:ObjectProperty';
