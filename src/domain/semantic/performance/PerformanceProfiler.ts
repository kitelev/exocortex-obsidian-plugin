...
export class GraphPerformanceProfiler {
   ...
   */
  profile<T>(operation: string, fn: () => T, metadata?: Record<string, any>): T {
  ...
  private calculateMetrics(): PerformanceMetrics[] {
    ...
    for (const [operation, durations] of operationGroups) {
      ...
      // Standard deviation
      const variance = durations.reduce((sum, d) => sum + Math.pow(d - avgTime, 2), 0) / count;
  ...
  private identifyHotspots(): string[] {
    ...
    for (const metric of metrics) {
      if (metric.totalTime > threshold || metric.avgTime > 5.0) {
        hotspots.push(`${metric.operation} (${metric.totalTime.toFixed(2)}ms total, ${metric.avgTime.toFixed(2)}ms avg)`);
      }
    }
...
export function profileMethod(operation?: string) {
  return function (target: any, propertyKey: string, descriptor: PropertyDescriptor) {
    ...
    descriptor.value = function (...args: any[]) {
      return globalProfiler.profile(operationName, () => originalMethod.apply(this, args));
    };
...
export function profileAsyncMethod(operation?: string) {
  return function (target: any, propertyKey: string, descriptor: PropertyDescriptor) {
    ...
    descriptor.value = async function (...args: any[]) {
      return globalProfiler.profileAsync(operationName, () => originalMethod.apply(this, args));
    };
