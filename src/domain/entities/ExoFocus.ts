...
export interface FocusFilter {
    ...
    operator: 'includes' | 'excludes' | 'equals' | 'contains' | 'before' | 'after' | 'between';
    value: any;
    property?: string;
}
...
export class ExoFocus extends Entity<ExoFocusProps> {
    ...
    public matchesTriple(triple: { subject: string; predicate: string; object: string }): boolean {
        if (!this.props.active) {
            return true;
        }
    ...
    private evaluateValue(operator: string, actualValue: any, filterValue: any): boolean {
        switch (operator) {
            case 'equals':
    ...
    private evaluateDateValue(operator: string, date: Date, filterValue: any): boolean {
        switch (operator) {
            case 'before':
