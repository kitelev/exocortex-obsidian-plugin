...
export class ExocortexSettings {
     ...
     */
    public static create(data: Partial<ExocortexSettingsData> = {}): Result<ExocortexSettings> {
        try {
            ...
            return Result.fail<ExocortexSettings>(`Failed to create settings: ${error.message}`);
        }
    }
    ...
    public validate(): Result<void> {
        ...
        if (!this.data.layoutsFolderPath || this.data.layoutsFolderPath.trim().length === 0) {
            return Result.fail<void>('Layouts folder path cannot be empty');
        }
        
        if (!this.data.templatesFolderPath || this.data.templatesFolderPath.trim().length === 0) {
     ...
     */
    public get<K extends keyof ExocortexSettingsData>(key: K): ExocortexSettingsData[K] {
    ...
    public static fromJSON(json: string): Result<ExocortexSettings> {
        try {
            ...
            return Result.fail<ExocortexSettings>(`Failed to parse settings JSON: ${error.message}`);
        }
    }
}