...
export class TouchClassTreeModal extends MobileModalAdapter {
  ...
  constructor(
    ...
    this.virtualScrolling =
      PlatformDetector.isMobile() && this.countTotalNodes() > 50;
  }
   ...
   */
  private convertToTouchTreeNodes(
    data: any[],
    level: number = 0,
    parent?: TouchTreeNode,
  ): TouchTreeNode[] {
    return data.map((item) => {
      ...
      if (item.children && item.children.length > 0) {
        node.children = this.convertToTouchTreeNodes(
          item.children,
          level + 1,
          node,
        );
  ...
  private setupSearchSection(container: HTMLElement): void {
    ...
    const searchContainer = searchSection.querySelector(
      ".form-section-content",
    ) as HTMLElement;

    // Create search input wrapper
    const inputWrapper = searchContainer.createDiv({
      cls: "touch-search-wrapper",
    });
   ...
   */
  private findFirstSelectableNode(
    nodes: TouchTreeNode[],
  ): TouchTreeNode | null {
    for (const node of nodes) {
      if (!node.isRecursion) {
  ...
  private updateVisibleNodes(): void {
    ...
    const virtualContainer = this.treeContainer.createDiv({
      cls: "virtual-tree-container",
    });
   ...
   */
  private renderRecursionNode(
    node: TouchTreeNode,
    container: HTMLElement,
  ): void {
    const nodeEl = container.createDiv({
      cls: "touch-tree-node touch-tree-recursion",
    });
  ...
  private renderTouchTreeNode(
    ...
    const elementData: TreeNodeElement = {
      node,
      element: nodeEl,
      contentElement: nodeEl.querySelector(
        ".touch-tree-node-content",
      ) as HTMLElement,
    };
   ...
   * Add expand/collapse functionality
   */
  private addExpandCollapseFunctionality(
    contentEl: HTMLElement,
    node: TouchTreeNode,
  ): void {
    const expandIcon = contentEl.querySelector(
      ".touch-tree-expand-icon",
    ) as HTMLElement;
   ...
   * Add selection functionality
   */
  private addSelectionFunctionality(
    contentEl: HTMLElement,
    node: TouchTreeNode,
  ): void {
