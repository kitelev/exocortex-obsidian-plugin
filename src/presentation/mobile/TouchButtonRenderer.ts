...
export class TouchButtonRenderer {
  ...
  async render(
    ...
    const buttonContainer = this.createButtonContainer(
      container,
      response.displayOptions.position,
    );
    ...
    if (groupedButtons.secondary.length > 0) {
      if (groupedButtons.secondary.length > 3 && PlatformDetector.isMobile()) {
        this.renderCollapsibleButtonGroup(
          buttonContainer,
          groupedButtons.secondary,
          assetId,
          context,
        );
      } else {
        const secondaryGroup = this.createButtonGroup(
          buttonContainer,
          "secondary",
        );
  ...
  private createButtonGroup(
    ...
    const isVertical =
      PlatformDetector.isMobile() || PlatformDetector.isTablet();
    ...
    if (!PlatformDetector.hasTouch()) {
      ...
      button.addEventListener("mouseleave", () => {
        ...
        button.style.boxShadow =
          variant === "primary" ? "0 2px 8px rgba(0, 0, 0, 0.1)" : "none";
      });
    }
  }
...
class TouchCommandInputModal extends Modal {
  ...
  onOpen(): void {
    ...
    const touchButtonRenderer = new TouchButtonRenderer(
      this.app,
      null as any,
      null as any,
    );
  ...
  private createTouchParameterInput(
    container: HTMLElement,
    param: CommandParameter,
  ): void {
    const inputContainer = container.createDiv({
      cls: "touch-input-container",
    });
  ...
  private createInputField(
    ...
    switch (param.type) {
      ...
      case "number": {
        ...
        numberInput.addEventListener("input", (e) => {
          this.inputValues[param.name] = Number(
    ...
    const inputs = inputWrapper.querySelectorAll(
