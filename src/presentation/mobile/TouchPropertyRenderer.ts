...
export class TouchPropertyRenderer {
    ...
    async renderPropertiesBlock(
        ...
        this.renderCustomPropertiesSection(propertiesEl, assetId, properties, currentProperties);
    }
    ...
    private groupPropertiesByCategory(properties: any[]): Map<string, any[]> {
        ...
        properties.forEach(prop => {
            const category = prop.category || (prop.isRequired ? 'Required' : 'Optional');
    ...
    private renderPropertyGroup(
        ...
        const groupContainer = this.createExpandableSection(container, category, true);
        const contentContainer = groupContainer.querySelector('.exocortex-expandable-content') as HTMLElement;
    ...
    private renderTouchProperty(
        ...
        const valueContainer = propertyEl.createDiv({ cls: 'touch-property-value' });
        
        if (this.editingProperty === property.propertyName) {
            this.renderTouchEditControl(valueContainer, assetId, property, currentValue);
        } else {
            this.renderTouchReadOnlyValue(valueContainer, assetId, property, currentValue);
        }
     ...
     */
    private createPropertyHeader(container: HTMLElement, property: any): HTMLElement {
        const header = container.createDiv({ cls: 'touch-property-header' });
        
        const labelContainer = header.createDiv({ cls: 'touch-property-label-container' });
    ...
    private renderTouchReadOnlyValue(
        ...
        const valueEl = container.createDiv({ cls: 'touch-property-value-readonly' });
        ...
        if (PlatformDetector.hasTouch()) {
            ...
            valueEl.addEventListener('touchend', () => {
                setTimeout(() => {
     ...
     */
    private showModalEditor(container: HTMLElement, assetId: string, property: any, currentValue: any): void {
        const modal = new MobileModalAdapter(
            this.app,
            {
            ...
            const buttonGroup = modal.createButtonGroup([
                ...
                },
                {
                    ...
                    onClick: () => {
                        ...
                        const modalContainer = modal.modalEl.querySelector('.touch-property-edit-form') as HTMLElement;
                        this.cancelTouchEdit(modalContainer || container, assetId, property);
                        modal.close();
                    }
                }
            ]);
    ...
    private renderTouchBooleanToggle(
        ...
        const toggleContainer = container.createDiv({ cls: 'touch-toggle-container' });
     ...
     */
    private async saveTouchProperty(assetId: string, property: any): Promise<void> {
    ...
    private renderCustomPropertiesSection(
        ...
        const schemaPropertyNames = new Set(schemaProperties.map(p => p.propertyName));
        const customProperties = Object.entries(currentProperties)
            .filter(([key]) => !schemaPropertyNames.has(key) && !key.startsWith('exo__'));

        if (customProperties.length > 0) {
            const customSection = this.createExpandableSection(container, 'Custom Properties', false);
            const contentContainer = customSection.querySelector('.exocortex-expandable-content') as HTMLElement;
            ...
            customProperties.forEach(([key, value]) => {
                ...
                this.renderTouchProperty(contentContainer, assetId, customProperty, value);
            });
        }
    }
     ...
     */
    private renderPropertyDescription(container: HTMLElement, description: string): void {
        if (PlatformDetector.isMobile()) {
            // Collapsible description on mobile
            const descSection = this.createExpandableSection(container, 'Description', false);
            const content = descSection.querySelector('.exocortex-expandable-content') as HTMLElement;
            content.createEl('p', {
                text: description,
     ...
     */
    private showTouchNotification(message: string, type: 'success' | 'error' | 'info'): void {
    ...
    private renderTouchObjectPropertyDropdown(container: HTMLElement, property: any, currentValue: any): void {
        // Implementation for object property dropdown
    }

    private renderTouchEnumSelector(container: HTMLElement, property: any, currentValue: any): void {
        // Implementation for enum selector
    }

    private renderTouchArrayEditor(container: HTMLElement, property: any, currentValue: any): void {
        // Implementation for array editor
    }

    private renderTouchTextArea(container: HTMLElement, property: any, currentValue: any): void {
