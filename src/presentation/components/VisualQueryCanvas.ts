...
export class VisualQueryCanvas {
  ...
  private createControls(): void {
    ...
    if (this.options.enableTemplates && this.options.templateUseCase) {
      const templatesBtn = this.createControlButton("Templates", () =>
        this.showTemplateSelection(),
      );
      const saveTemplateBtn = this.createControlButton("Save Template", () =>
      ...
      templateSeparator.style.cssText =
        "width: 1px; background: var(--background-modifier-border); margin: 0 4px;";
      controls.appendChild(templateSeparator);
    }

    const addEntityBtn = this.createControlButton("Entity", () =>
      this.addNode(NodeType.ENTITY),
    );
    const addVariableBtn = this.createControlButton("Variable", () =>
      this.addNode(NodeType.VARIABLE),
    );
    const addLiteralBtn = this.createControlButton("Literal", () =>
      this.addNode(NodeType.LITERAL),
    );
    const addFilterBtn = this.createControlButton("Filter", () =>
    ...
    const clearBtn = this.createControlButton("Clear", () =>
      this.clearCanvas(),
    );
  ...
  private handleWheel(e: WheelEvent): void {
    ...
    const newZoom = Math.max(
      ...
      Math.min(this.options.zoomMax, this.zoom + delta),
    );
  ...
  private showNodeContextMenu(nodeId: string, position: NodePosition): void {
    ...
    const deleteOption = this.createMenuOption("Delete", () => {
      ...
      this.edges.forEach((edge, id) => {
        if (
          edge.getSourceNodeId() === nodeId ||
          edge.getTargetNodeId() === nodeId
        ) {
          this.edges.delete(id);
        }
      });
  ...
  private renderNodes(): void {
    ...
    this.nodes.forEach((node) => {
      ...
      rect.setAttribute(
        "stroke",
        node.isSelected() ? "var(--text-accent)" : colors.stroke,
      );
  ...
  private renderEdges(): void {
    ...
    this.edges.forEach((edge) => {
      ...
      path.setAttribute(
        "stroke",
        edge.isSelected() ? "var(--text-accent)" : "var(--text-muted)",
      );
  ...
  private getNodeColors(type: NodeType): {
    fill: string;
    stroke: string;
    text: string;
  } {
    ...
    try {
      if (template.getParameters().length > 0) {
        ...
        if (!this.options.appInstance) {
          new Notice(
            "Cannot configure template parameters - app instance not available",
          );
          return;
        }
  ...
  private async instantiateTemplate(template: QueryTemplate): Promise<void> {
    try {
      const { nodes, edges } =
        await this.options.templateUseCase!.instantiateTemplate(template);
      ...
      new Notice(
        `Template "${template.getMetadata().name}" loaded successfully`,
      );
    } catch (error) {
  ...
  private showSaveTemplateModal(): void {
    ...
    const saveModal = new SaveTemplateModal(this.options.appInstance, {
      nodes: this.nodes,
      edges: this.edges,
      viewport: {
        x: this.viewportX,
      ...
      onSave: async (templateData) => {
        try {
          const template =
            await this.options.templateUseCase!.createCustomTemplate(
              this.nodes,
              this.edges,
              {
                x: this.viewportX,
          ...
          new Notice(
            `Template "${template.getMetadata().name}" saved successfully`,
          );
        } catch (error) {
  ...
  exportToJSON(): string {
    const canvasData = {
      ...
      nodes: Array.from(this.nodes.entries()).map(([id, node]) => ({
        ...
        uri: (node as any).getEntityUri
          ? (node as any).getEntityUri()
          : undefined,
      })),
