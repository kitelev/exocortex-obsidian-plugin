 ...
 * Touch Gesture Types
 */
export type TouchGestureType = 'tap' | 'doubletap' | 'longtap' | 'swipe' | 'pinch';
...
export class MobileUIComponents {
    ...
    public createButton(
        ...
        if (config.loading) {
            ...
            const spinner = button.createEl('span', { cls: 'exocortex-mobile-spinner' });
            spinner.innerHTML = '⟳';
        }

        if (config.icon) {
            const icon = button.createEl('span', { cls: 'exocortex-mobile-button-icon' });
    ...
    public createInput(
        ...
        const inputContainer = container.createEl('div', { cls: 'exocortex-mobile-input-container' });
        ...
        if (onChange) {
            const debouncedOnChange = this.performanceOptimizer?.debounce(
                onChange,
                300
            ) || onChange;
    ...
    public createModal(
        ...
        const modal = document.body.createEl('div', { cls: 'exocortex-mobile-modal-overlay' });
        ...
        // Modal header
        const header = modalContent.createEl('div', { cls: 'exocortex-mobile-modal-header' });
        ...
        // Modal body
        const body = modalContent.createEl('div', { cls: 'exocortex-mobile-modal-body' });
    ...
    public createLoadingIndicator(
        ...
        const loader = container.createEl('div', { cls: 'exocortex-mobile-loader' });
        
        const spinner = loader.createEl('div', { cls: 'exocortex-mobile-spinner-large' });
    ...
    private createListItem(
        ...
        const content = listItem.createEl('div', { cls: 'exocortex-mobile-list-item-content' });
        ...
        const textContainer = content.createEl('div', { cls: 'exocortex-mobile-list-item-text' });
        ...
        const rightContainer = content.createEl('div', { cls: 'exocortex-mobile-list-item-right' });
    ...
    private addSwipeActions(
        ...
        actions: Array<{ text: string; icon?: string; color?: string; action: () => void }>
    ): void {
    ...
    private closeModal(modal: HTMLElement): void {
        // Add exit animation
        modal.style.opacity = '0';
        const modalContent = modal.querySelector('.exocortex-mobile-modal') as HTMLElement;
        if (modalContent) {
            modalContent.style.transform = 'translateY(100%)';
        }
