...
import { ExocortexError, ErrorSeverity, ErrorCategory, ErrorBuilder } from '../../domain/errors/ExocortexError';
...
export class EnhancedSPARQLProcessor extends SPARQLProcessor {
    ...
    private validateQuery(query: string): ExocortexError | null {
                                ...
                                .withSuggestions([
                                    {
                                        ...
                                        description: 'Ensure all opening parentheses ( have corresponding closing parentheses )',
                                        confidence: 0.9
                                    }
                                ])
                                .withRecoverable(true)
        ...
        const commonPatterns = [
            { pattern: /SELECT\s+WHERE/i, message: 'Missing variable selection after SELECT' },
            { pattern: /WHERE\s*$/i, message: 'WHERE clause is empty' },
            { pattern: /FILTER\s*$/i, message: 'FILTER expression is incomplete' },
            { pattern: /PREFIX\s+\w+\s*$/i, message: 'PREFIX declaration missing IRI' }
        ];
    ...
    private convertToEnhancedError(error: Error | any, query: string): ExocortexError {
        ...
        if (lineMatch || columnMatch) {
            baseError.context.location = {
                ...
                context: lineMatch ? this.getLineContext(query, parseInt(lineMatch[1])) : undefined
            };
        }
        ...
        if (errorMessage.includes('timeout')) {
            suggestions.push({
                ...
                action: {
                    label: 'Add LIMIT 100',
                    handler: () => {
                        const limitedQuery = query.trim().replace(/;?\s*$/, '') + '\nLIMIT 100';
    ...
    private displayValidationError(container: HTMLElement, error: ExocortexError, query: string): void {
    ...
    private addQueryHighlighting(container: HTMLElement, query: string, error: ExocortexError): void {
        ...
        highlightedQuery.style.cssText = 'margin-top: 1em; padding: 0.5em; background: #fff; border: 1px solid #e0e0e0; border-radius: 4px;';
        
        const title = document.createElement('div');
        title.textContent = 'Query with Error Location:';
        title.style.cssText = 'font-weight: bold; margin-bottom: 0.5em; color: #666;';
        highlightedQuery.appendChild(title);
        
        const pre = document.createElement('pre');
        pre.style.cssText = 'margin: 0; font-family: monospace; font-size: 0.9em; line-height: 1.4;';
        ...
        lines.forEach((line, index) => {
            ...
            if (location.line === lineNumber) {
                ...
                if (location.column) {
                    ...
                    errorChar.style.cssText = 'background: #f00; color: #fff; padding: 0 2px; font-weight: bold;';
        ...
        if (location.column) {
            ...
            pointer.style.cssText = 'margin-left: 3.5em; color: #f00; font-size: 0.8em;';
    ...
    private displayResults(container: HTMLElement, source: string, result: any, executionTime: number): void {
        ...
        if (!result.results || result.results.length === 0) {
            const noResultsError = ErrorBuilder.create()
                ...
                .withMessage('Your query executed successfully but returned no results.')
                ...
                .withSuggestions([
                    {
                        ...
                        description: 'Ensure your triple patterns match existing data in the graph',
                        confidence: 0.6
                    },
                    {
