import { Command, Notice, TFile, App, Modal, Setting, ProgressBarComponent } from 'obsidian';
...
class OntologizeOptionsModal extends Modal {
    ...
    onOpen(): void {
        ...
        benefitsList.createEl('li', { text: 'Beautiful Graph View with relation nodes' });
        benefitsList.createEl('li', { text: 'Flexible access control per relation' });
    ...
    private async ontologizeCurrentFile(): Promise<void> {
        try {
            const relations = await this.ontologizer.ontologizeAsset(this.currentFile);
...
class OntologizeVaultModal extends Modal {
    ...
    onOpen(): void {
        ...
        processList.createEl('li', { text: 'Extract object properties from frontmatter' });
        processList.createEl('li', { text: 'Create Relation assets in "99 Relations" folder' });
        processList.createEl('li', { text: 'Clean original assets (remove converted properties)' });
        
        // Progress container
        const progressContainer = contentEl.createDiv({ cls: 'ontologize-progress' });
