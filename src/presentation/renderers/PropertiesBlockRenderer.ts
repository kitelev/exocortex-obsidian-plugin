...
export class PropertiesBlockRenderer {
  ...
  async render(
    ...
    if (
      propsConfig.includedProperties &&
      propsConfig.includedProperties.length > 0
    ) {
    ...
    if (
      propsConfig.excludedProperties &&
      propsConfig.excludedProperties.length > 0
    ) {
  ...
  private async renderFlatProperties(
    ...
    const editableContainer = container.createDiv({
    ...
    if (readOnlyProps.length > 0) {
      const readOnlyContainer = container.createDiv({
        cls: "exocortex-properties-readonly",
      });
      const table = readOnlyContainer.createEl("table", {
        cls: "exocortex-properties-table",
      });
  ...
  private renderGroupedProperties(
    ...
    groups.forEach((props, groupName) => {
      ...
      if (groupReadOnly.length > 0) {
        const table = groupContent.createEl("table", {
          cls: "exocortex-properties-table",
        });
