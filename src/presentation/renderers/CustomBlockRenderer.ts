...
import {
  CustomBlockConfig,
  QueryEngineQuery,
} from "../../domain/entities/LayoutBlockStubs";
  ...
  async render(
    ...
    try {
      ...
      if (customConfig.queryEngineQuery) {
        await this.renderQueryEngineQuery(
          container,
          customConfig.queryEngineQuery,
          file,
          frontmatter,
        );
        return;
      ...
      if (customConfig.dataviewQuery) {
        await this.renderLegacyDataviewQuery(
          container,
          customConfig.dataviewQuery,
          file,
          frontmatter,
          dv,
        );
      ...
      if (customConfig.customScript) {
        await this.renderCustomScript(
          container,
          customConfig.customScript,
          file,
          frontmatter,
          dv,
        );
