...
import { DynamicBacklinksService, PropertyBasedBacklink } from '../../application/services/DynamicBacklinksService';
...
export class DynamicBacklinksBlockRenderer {
    ...
    async render(
        ...
        const backlinkResult = await this.dynamicBacklinksService.discoverPropertyBasedBacklinks(file, {
            excludeProperties: config.excludeProperties || ['exo__Asset_id', 'exo__Instance_class'],
        ...
        for (const propertyGroup of propertyBacklinks) {
            if (!config.showEmptyProperties && propertyGroup.referencingFiles.length === 0) {
                continue;
            }
    ...
    private async renderBacklinkItem(list: HTMLElement, file: TFile): Promise<void> {
