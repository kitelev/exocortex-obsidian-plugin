...
import { RDFSerializer, RDFFormat, SerializationOptions } from '../../application/services/RDFSerializer';
...
export class ExportRDFModal extends Modal {
    ...
    private onExport?: (result: { content: string; fileName: string; format: RDFFormat }) => void;
    
    constructor(
        app: App, 
        graph: Graph, 
        namespaceManager?: NamespaceManager,
        onExport?: (result: { content: string; fileName: string; format: RDFFormat }) => void
    ) {
        super(app);
        this.graph = graph;
    ...
    private updatePreview(): void {
        const previewEl = this.contentEl.querySelector('.export-preview-content') as HTMLElement;
        if (!previewEl) return;
    ...
    private async handleExport(): Promise<void> {
        try {
            ...
            if (serializedData.metadata?.warnings?.length) {
                new Notice(`Warnings: ${serializedData.metadata.warnings.join('; ')}`, 5000);
            }
