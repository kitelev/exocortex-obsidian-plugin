...
export class TemplateParameterModal extends Modal {
  ...
  private createParameterInput(
    ...
    input.addEventListener("input", () =>
      this.validateParameter(parameter, input, errorContainer),
    );
  ...
  private createInputElement(
    ...
    if (
      ...
      (!parameter.constraints?.maxLength ||
        parameter.constraints.maxLength > 100)
    ) {
    ...
    if (parameter.type === "entity" || parameter.type === "property") {
      input.placeholder =
        parameter.type === "entity"
          ? "e.g., Person, ex:John, <http://example.org/Person>"
          : "e.g., name, ex:hasAge, ?property";
    }

    return input;
  }

  private createConstraintsHelp(
    container: HTMLElement,
  ...
  private validateParameter(
    ...
    if (parameter.required && !value) {
      ...
      this.validationErrors.set(
        parameter.id || `param_${parameter.name}`,
        error,
      );
