...
export class ClassTreeModal extends Modal {
    ...
    private convertToTreeNodes(data: any[], level: number = 0): TreeNode[] {
        return data.map(item => ({
            ...
            children: item.children ? this.convertToTreeNodes(item.children, level + 1) : [],
    ...
    async onOpen() {
        ...
        // Search bar
        const searchContainer = contentEl.createEl('div', { cls: 'class-tree-search' });
        ...
        // Tree container
        this.treeContainer = contentEl.createEl('div', { cls: 'class-tree-container' });
        this.renderTree();
    ...
    private filterTree(searchTerm: string) {
        ...
        const filterNodes = (nodes: TreeNode[]): TreeNode[] => {
            ...
            for (const node of nodes) {
                const matchesSearch = node.className.toLowerCase().includes(term) || 
    ...
    private updateNodeInTree(nodes: TreeNode[], className: string, isExpanded: boolean): boolean {
        for (const node of nodes) {
