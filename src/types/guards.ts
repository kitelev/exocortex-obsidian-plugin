...
export function isObject(value: unknown): value is Record<string, unknown> {
  return value !== null && typeof value === 'object' && !isArray(value) && !isDate(value);
}
...
export function isPropertyType(value: string): value is PropertyType {
  const validTypes: PropertyType[] = ['string', 'number', 'boolean', 'date', 'array', 'enum', 'text', 'object'];
...
export function validatePropertyValue(value: unknown, type: PropertyType): PropertyValue {
  switch (type) {
    case 'string':
    case 'text':
    ...
    case 'number':
      if (value === null || value === undefined) return value;
      if (!isNumber(value)) throw new TypeError(`Expected number, got ${typeof value}`);
      return value;
      
    case 'boolean':
      if (value === null || value === undefined) return value;
      if (!isBoolean(value)) throw new TypeError(`Expected boolean, got ${typeof value}`);
      return value;
    ...
    case 'date':
      ...
      if (isString(value)) {
        ...
        if (isNaN(date.getTime())) throw new TypeError(`Invalid date string: ${value}`);
        return date;
      }
      if (!isDate(value)) throw new TypeError(`Expected date, got ${typeof value}`);
      return value;
      
    case 'array':
      if (value === null || value === undefined) return value;
      if (!isArray(value)) throw new TypeError(`Expected array, got ${typeof value}`);
      return value as PropertyValue;
      
    case 'object':
      if (value === null || value === undefined) return value;
      if (!isObject(value)) throw new TypeError(`Expected object, got ${typeof value}`);
      return value;
      
    case 'enum':
      if (value === null || value === undefined) return value;
      if (!isString(value)) throw new TypeError(`Expected enum string, got ${typeof value}`);
      return value;
