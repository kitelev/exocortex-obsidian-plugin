...
export class ObsidianClassViewRepository implements IClassViewRepository {
    ...
    async findByClassName(className: ClassName): Promise<Result<ClassView | null>> {
        try {
            ...
            return Result.fail<ClassView | null>(`Failed to find ClassView: ${error.message}`);
        }
    }
    ...
    async findById(id: AssetId): Promise<Result<ClassView | null>> {
        try {
            ...
            return Result.fail<ClassView | null>(`Failed to find ClassView by ID: ${error.message}`);
        }
    }
    ...
    async findAll(): Promise<Result<ClassView[]>> {
        try {
            ...
            return Result.fail<ClassView[]>(`Failed to find all ClassViews: ${error.message}`);
        }
    }
    ...
    private async buildClassViewFromFile(file: TFile): Promise<Result<ClassView | null>> {
        try {
            const metadata = this.app.metadataCache.getFileCache(file);
            ...
            return Result.fail<ClassView | null>(`Failed to build ClassView: ${error.message}`);
        }
    }

    private async loadButton(buttonName: string): Promise<Result<UIButton | null>> {
        try {
            ...
            return Result.fail<UIButton | null>(`Failed to load button: ${error.message}`);
        }
    }
