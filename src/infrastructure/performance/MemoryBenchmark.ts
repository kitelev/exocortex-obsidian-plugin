...
export class MemoryBenchmark {
  ...
  async benchmarkOptimizedVsStandard(): Promise<BenchmarkResult> {
    ...
    const memoryReport = optimizedResult.isSuccess
      ? optimizedResult.getValue()
      : null;
  ...
  private async runBenchmark(
    ...
    // Import data
    const importResult = await this.importer.importRDF(
      testData,
      graph,
      options,
    );
    ...
    const memoryReport = importResult.isSuccess
...
`;
  ...
  private generateSummary(
    ...
    const avgMemoryReduction =
      results.reduce((sum, r) => sum + r.memoryUsage.reduction, 0) /
      results.length;
    const avgThroughput =
      results.reduce((sum, r) => sum + r.performance.throughput, 0) /
      results.length;
...
`;
    ...
    console.log(
      `  Memory used: ${Math.round((endMemory - startMemory) / 1024)} KB`,
    );
