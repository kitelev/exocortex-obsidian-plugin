...
export class DatacoreQueryEngine implements IQueryEngine {
    ...
    public validateQuery(query: string): Result<boolean> {
        ...
        if (!hasValidDataviewStart && !hasDatacorePattern) {
            return Result.fail<boolean>('Query must be valid Dataview or Datacore syntax');
        }
    ...
    private async executeNativeQuery(query: string, context?: QueryContext): Promise<QueryResult> {
    ...
    private async renderDatacoreQuery(
        ...
        const dcContainer = container.createDiv({ cls: 'exocortex-datacore-container' });
