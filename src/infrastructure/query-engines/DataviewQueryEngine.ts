...
export class DataviewQueryEngine implements IQueryEngine {
    ...
    public async getPageMetadata(path: string): Promise<Result<Record<string, any>>> {
        if (!this.isAvailable()) {
        ...
        try {
            ...
            return Result.fail<Record<string, any>>(`Failed to get page metadata: ${error}`);
        }
    }
    ...
    public validateQuery(query: string): Result<boolean> {
        ...
        if (!hasValidStart && !trimmedQuery.includes('dv.')) {
            return Result.fail<boolean>('Query must start with table, list, task, calendar, or contain dv. calls');
        }
    ...
    private async executeTableQuery(query: string, context?: QueryContext): Promise<QueryResult> {
    ...
    private async executeListQuery(query: string, context?: QueryContext): Promise<QueryResult> {
        const fromMatch = query.match(/from\s+(.+?)(?:\s+where|$)/s);
    ...
    private async executeTaskQuery(query: string, context?: QueryContext): Promise<QueryResult> {
        const fromMatch = query.match(/from\s+(.+?)(?:\s+where|$)/s);
        
        const source = fromMatch ? fromMatch[1].trim() : '""';
        const tasks = this.dataviewApi.pages(source)
            .file.tasks
            .where((t: any) => !t.completed);
    ...
    private async executeCalendarQuery(query: string, context?: QueryContext): Promise<QueryResult> {
    ...
    private async executeRawQuery(query: string, context?: QueryContext): Promise<QueryResult> {
    ...
    private async renderTableQuery(container: HTMLElement, query: string): Promise<void> {
        const result = await this.executeTableQuery(query);
        
        if (result.data.length === 0) {
            container.createEl('p', { text: 'No results found', cls: 'exocortex-empty' });
            return;
        }

        this.dataviewApi.table(
            result.columns,
            result.data.map((row: any) => result.columns!.map(col => row[col] || ''))
        );
    }

    private async renderListQuery(container: HTMLElement, query: string): Promise<void> {
    ...
    private async renderTaskQuery(container: HTMLElement, query: string): Promise<void> {
        const result = await this.executeTaskQuery(query);
        
        if (result.data.length === 0) {
            container.createEl('p', { text: 'No tasks found', cls: 'exocortex-empty' });
            return;
        }
