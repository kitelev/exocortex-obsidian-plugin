...
export class ExocortexAPIServer {
    ...
    async start(): Promise<void> {
        ...
        try {
            ...
            new Notice(`🌐 Exocortex REST API started on port ${this.settings.useHttps ? this.settings.securePort : this.settings.port}`);
            console.log(`API Key: ${this.apiKey}`);
            console.log(`API Documentation: http://localhost:${this.settings.port}/api/docs`);
        } catch (error) {
    ...
    private async startHttpServer(): Promise<void> {
        try {
            ...
            console.error('Failed to start HTTP server. Using file-based API fallback.', error);
        ...
        try {
            ...
            return new Promise((resolve, reject) => {
                this.httpsServer.listen(this.settings.securePort, () => {
                    console.log(`HTTPS server listening on port ${this.settings.securePort}`);
                    resolve();
                });
            ...
            console.error('Failed to start HTTPS server. Using file-based API fallback.', error);
            await this.startFileBasedAPI();
        }
    }
    ...
    private async handleOntologize(
        req: any,
        res: any
    ): Promise<void> {
        if (req.method !== 'POST') {
    ...
    private async handleFocus(
        ...
        if (req.method === 'GET') {
            ...
            await this.app.vault.adapter.write(focusPath, JSON.stringify(focusData, null, 2));
    ...
    private generateSelfSignedCertificate(): { privateKey: string; certificate: string } {
        return {
