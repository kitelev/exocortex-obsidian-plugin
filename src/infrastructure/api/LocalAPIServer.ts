...
${this.getEndpointList().map(e => `- ${e.method} ${e.path}: ${e.description}`).join('\n')}
...
`;
    ...
    private async handleCreateAsset(params: any): Promise<any> {
        ...
        const uid = 'asset-' + Date.now() + '-' + Math.random().toString(36).substring(2, 9);
    ...
    private async handleGetFocus(): Promise<any> {
        try {
            const content = await this.app.vault.adapter.read('.exocortex-api/focus.json');
            return {
                status: 200,
    ...
    private getEndpointList(): any[] {
        return [
            ...
            { method: 'POST', path: '/graph', description: 'Graph operations (match/add)' },
            { method: 'POST', path: '/assets/search', description: 'Search assets' },
            { method: 'POST', path: '/assets/create', description: 'Create new asset' },
