...
export class SPARQLSecurityManager {
  ...
  async validateQuery(
    ...
    try {
      ...
      // 3. Check rate limits
      const isComplex =
        complexity.metrics.riskLevel === "high" ||
        complexity.metrics.riskLevel === "critical";
      ...
      if (!rateLimitResult.isSuccess) {
        this.monitor.logRateLimitExceeded(
          userId,
          "query_check",
          0,
          0,
      ...
      if (!validation.allowed) {
        allViolations.push(
          ...validation.detectedThreats.map((t) => t.description),
        );
      }
      ...
      if (!rateLimit.allowed) {
        recommendations.push(
          `Wait ${Math.round(rateLimit.retryAfterMs! / 1000)} seconds before retry`,
        );
      }
  ...
  async executeQueryWithSecurity(
    ...
    const complexityLevel =
      validation.metrics.complexity.riskLevel === "critical"
        ? "critical"
        : validation.metrics.complexity.riskLevel === "high"
          ? "complex"
          : validation.metrics.complexity.riskLevel === "medium"
            ? "moderate"
            : "simple";
    ...
    if (!executionResult.isSuccess) {
      throw new Error(
        `Failed to start query execution: ${executionResult.getError()}`,
      );
    }
  ...
  getSecurityStatus(): {
    ...
    if (
      monitorMetrics.criticalIncidentsLast24h > 0 ||
      rateLimiterStats.circuitBreakersOpen > 0
    ) {
      systemHealth = "critical";
    } else if (
      monitorMetrics.eventsBySeverity.high > 10 ||
      timeoutStats.timeoutRate > 0.1
    ) {
    ...
    return {
      ...
      activeThreats:
        monitorMetrics.criticalIncidentsLast24h +
        monitorMetrics.eventsBySeverity.high,
      systemHealth,
    };
  }
  ...
  private generateSystemRecommendations(): string[] {
    ...
    if (status.systemHealth === "critical") {
      recommendations.push(
        "Immediate action required: System under attack or severe load",
      );
    ...
    if (status.rateLimiter.circuitBreakersOpen > 0) {
      recommendations.push(
        "Circuit breakers active - investigate user behavior",
      );
    }

    if (status.timeoutManager.timeoutRate > 0.05) {
      recommendations.push(
        "High timeout rate detected - review query complexity limits",
      );
    }
