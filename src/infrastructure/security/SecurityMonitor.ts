...
export class SecurityMonitor {
  ...
  constructor(config: Partial<MonitorConfig> = {}) {
    ...
    setInterval(
      ...
      this.mergedConfig.correlationWindowMs,
    );
  }
    ...
    try {
      const event: SecurityEvent = {
        ...
        details: {
          ...details,
          actionTaken:
            details.actionTaken || this.determineActionTaken(type, severity),
        },
        metadata: {
          ...metadata,
          tags:
            metadata.tags || this.generateEventTags(type, severity, details),
        },
      };
    ...
    return this.logSecurityEvent(
      ...
      },
      {
        ...
        metrics: {
          currentRate,
          limit,
          overagePercent: ((currentRate - limit) / limit) * 100,
        },
      },
      userId,
      sessionId,
  ...
  generateSecurityReport(timeRangeMs: number = 24 * 60 * 60 * 1000): {
    ...
    const criticalEvents = this.events.filter(
      (e) => e.timestamp > cutoff && e.severity === "critical",
    );
    ...
    try {
      const alert: SecurityAlert = {
        ...
        recommendation:
          ...
          this.generateRecommendationForSeverity(severity),
        acknowledged: false,
      };
  ...
  private shouldTriggerAlert(event: SecurityEvent): boolean {
    ...
    return (
      criticalCount >=
        this.mergedConfig.alertThresholds.criticalEventsPerMinute ||
  ...
  private correlateEvents(): void {
    ...
    for (const [userId, events] of userEventCounts.entries()) {
      if (
        events.length >=
        this.mergedConfig.alertThresholds.suspiciousPatternCount
      ) {
  ...
  private cleanup(): void {
    ...
    this.alerts.splice(
      ...
      this.alerts.length,
      ...this.alerts.filter(
        (a) => a.timestamp > cutoff || !a.resolvedAt || a.resolvedAt > cutoff,
      ),
    );

    console.log(
      `Security Monitor cleanup: removed ${oldEventCount - this.events.length} events, ${oldAlertCount - this.alerts.length} alerts`,
    );
  }
  ...
  private generateRecommendations(
    ...
    if (criticalEvents.some((e) => e.type === "injection_attempt")) {
      recommendations.push(
        "Enhance input validation and consider WAF deployment",
      );
    }
  ...
  private assessComplianceStatus(metrics: SecurityMetrics): string {
    if (
      metrics.criticalIncidentsLast24h === 0 &&
      metrics.eventsBySeverity.high < 5
    ) {
      return "COMPLIANT";
    }
