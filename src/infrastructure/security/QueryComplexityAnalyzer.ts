...
export class QueryComplexityAnalyzer {
  ...
  private calculateComplexityMetrics(query: string): ComplexityMetrics {
    ...
    const estimatedCost =
      baseCost + joinCost + filterCost + unionCost + subqueryCost;
    ...
    const estimatedExecutionTimeMs = this.estimateExecutionTime(
      estimatedCost,
      joinComplexity,
    );
   ...
   */
  private classifyMemoryComplexity(
    memoryMB: number,
  ): ComplexityMetrics["memoryComplexity"] {
  ...
  private assessRiskLevel(
    ...
    if (cost > 800 || timeComplexity === "O(2^n)" || memoryMB > 50)
      return "critical";
    if (cost > 500 || timeComplexity === "O(n²)" || memoryMB > 20)
      return "high";
    if (cost > 200 || timeComplexity === "O(n log n)" || memoryMB > 5)
      return "medium";
    return "low";
  }
  ...
  private checkViolations(metrics: ComplexityMetrics): string[] {
    ...
    if (metrics.estimatedCost > this.mergedThresholds.maxCost) {
      violations.push(
        `Estimated cost (${metrics.estimatedCost}) exceeds maximum (${this.mergedThresholds.maxCost})`,
      );
    }

    if (metrics.triplePatterns > this.mergedThresholds.maxTriplePatterns) {
      violations.push(
        `Triple patterns (${metrics.triplePatterns}) exceed maximum (${this.mergedThresholds.maxTriplePatterns})`,
      );
    }

    if (metrics.joinComplexity > this.mergedThresholds.maxJoinComplexity) {
      violations.push(
        `Join complexity (${metrics.joinComplexity}) exceeds maximum (${this.mergedThresholds.maxJoinComplexity})`,
      );
    }

    if (metrics.subqueryDepth > this.mergedThresholds.maxSubqueryDepth) {
      violations.push(
        `Subquery depth (${metrics.subqueryDepth}) exceeds maximum (${this.mergedThresholds.maxSubqueryDepth})`,
      );
    }

    if (
      metrics.estimatedMemoryMB > this.mergedThresholds.maxEstimatedMemoryMB
    ) {
      violations.push(
        `Estimated memory (${metrics.estimatedMemoryMB}MB) exceeds maximum (${this.mergedThresholds.maxEstimatedMemoryMB}MB)`,
      );
    }

    if (
      metrics.estimatedExecutionTimeMs >
      this.mergedThresholds.maxExecutionTimeMs
    ) {
      violations.push(
        `Estimated execution time (${metrics.estimatedExecutionTimeMs}ms) exceeds maximum (${this.mergedThresholds.maxExecutionTimeMs}ms)`,
      );
    }

    if (
      !this.mergedThresholds.allowedTimeComplexity.includes(
        metrics.timeComplexity,
      )
    ) {
      violations.push(
        `Time complexity (${metrics.timeComplexity}) not allowed`,
      );
    }
  ...
  private generateRecommendations(
    ...
    if (metrics.riskLevel === "critical" || metrics.riskLevel === "high") {
      recommendations.push(
        "This query poses security risks - consider major restructuring",
      );
    }
