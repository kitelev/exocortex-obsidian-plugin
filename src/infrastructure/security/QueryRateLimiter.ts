...
export class QueryRateLimiter {
  ...
  checkRateLimit(
    ...
    try {
      ...
      if (
        isComplex &&
        complexRequestCount >= adjustedLimits.maxComplexRequests
      ) {
        result.allowed = false;
      ...
      if (totalCost + queryCost > maxTotalCost) {
        result.allowed = false;
        result.retryAfterMs = this.calculateCostBasedRetry(
          totalCost,
          maxTotalCost,
        );
      }
  ...
  private getAdjustedLimits(metrics: UserMetrics): RateLimitConfig {
    ...
    if (metrics.violationCount > 0) {
      ...
      adjustedConfig.maxRequests = Math.floor(
        adjustedConfig.maxRequests * reductionFactor,
      );
      adjustedConfig.maxComplexRequests = Math.floor(
        adjustedConfig.maxComplexRequests * reductionFactor,
      );
    }
  ...
  private detectBurst(window: RequestWindow[], now: number): boolean {
    ...
    const recentRequests = window.filter(
      (req) => req.timestamp > now - burstWindow,
    );
  ...
  private calculateRetryAfter(
    ...
    const timeUntilExpiry =
      oldestRequest.timestamp + this.mergedConfig.windowSizeMs - now;
   ...
   */
  private calculateCostBasedRetry(
    currentCost: number,
    maxCost: number,
  ): number {
  ...
  private recordViolation(userId: string, timestamp: number): void {
    ...
    if (metrics.violationCount >= this.mergedConfig.circuitBreakerThreshold) {
      ...
      metrics.circuitBreakerResetTime =
        timestamp + this.mergedConfig.circuitBreakerResetTimeMs;
    }
  }
  ...
  private cleanup(): void {
    ...
    for (const [userId, metrics] of this.userMetrics.entries()) {
      if (
        metrics.lastRequestTime < cutoff &&
  ...
  setEmergencyMode(
    enabled: boolean,
    emergencyConfig?: Partial<RateLimitConfig>,
  ): void {
