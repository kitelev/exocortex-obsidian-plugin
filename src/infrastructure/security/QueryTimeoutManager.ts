...
export class QueryTimeoutManager {
    ...
    completeExecution(queryId: string): Result<QueryMetrics> {
        ...
        const metrics: QueryMetrics = {
            executionTimeMs,
            peakMemoryMB: Math.max(execution.resourceSnapshot.memoryUsageMB, finalSnapshot.memoryUsageMB),
            avgCpuPercent: (execution.resourceSnapshot.cpuUsagePercent + finalSnapshot.cpuUsagePercent) / 2,
            resourceViolations: this.checkResourceViolations(execution, finalSnapshot)
        };
     ...
     */
    cancelExecution(queryId: string, reason: QueryMetrics['timeoutReason'] = 'manual'): Result<void> {
        const execution = this.activeExecutions.get(queryId);
        if (!execution) {
            return Result.fail(`No active execution found for query ID: ${queryId}`);
        }
    ...
    private monitorExecution(queryId: string): void {
        ...
        if (violations.length > 0) {
            console.warn(`Resource violations detected for query ${queryId}:`, violations);
            
            // Cancel if critical violations
            const criticalViolations = violations.filter(v => 
                v.includes('memory') && currentSnapshot.memoryUsageMB > this.mergedConfig.memoryThresholdMB * 1.5 ||
                v.includes('CPU') && currentSnapshot.cpuUsagePercent > this.mergedConfig.cpuThresholdPercent * 1.2
            );

            if (criticalViolations.length > 0) {
                this.cancelExecution(queryId, violations.some(v => v.includes('memory')) ? 'memory' : 'cpu');
            }
        }
    }
    ...
    private checkResourceViolations(
        ...
        // CPU violations
        if (currentSnapshot.cpuUsagePercent > this.mergedConfig.cpuThresholdPercent) {
            violations.push(
        ...
        const memoryGrowth = currentSnapshot.memoryUsageMB - execution.resourceSnapshot.memoryUsageMB;
    ...
    private getSystemLoad(): SystemLoad {
        ...
        return {
            ...
            availableMemoryMB: Math.round((memUsage.heapTotal - memUsage.heapUsed) / 1024 / 1024),
    ...
    getExecutionStatistics(): {
        ...
        const timeouts = this.executionHistory.filter(m => m.timeoutReason).length;
        const resourceViolations = this.executionHistory.filter(m => m.resourceViolations.length > 0).length;
        
        const avgExecutionTime = this.executionHistory.reduce((sum, m) => sum + m.executionTimeMs, 0) / totalExecutions;
        const avgMemoryUsage = this.executionHistory.reduce((sum, m) => sum + m.peakMemoryMB, 0) / totalExecutions;
        ...
        return {
            ...
            resourceViolationRate: Math.round((resourceViolations / totalExecutions) * 100) / 100
        };
    }
