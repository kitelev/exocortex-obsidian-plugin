...
export class AgentNecessityAnalyzer {
  ...
  analyzeNeed(
    ...
    try {
      ...
      const decision = this.makeDecision(task, existingAgents, graspMetrics, solidMetrics);
      
      return Result.ok(decision);
    } catch (error) {
      return Result.fail(`Agent necessity analysis failed: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  private calculateGRASPMetrics(
    ...
    const domainExperts = existingAgents.filter(agent => 
  ...
  private calculateSOLIDMetrics(
    ...
    return {
      singleResponsibility: this.assessSingleResponsibility(task, existingAgents),
      openClosed: this.assessOpenClosed(task, existingAgents),
      liskovSubstitution: this.assessLiskovSubstitution(task, existingAgents),
      interfaceSegregation: this.assessInterfaceSegregation(task, existingAgents),
  ...
  private makeDecision(
    ...
    const graspScore = Object.entries(graspMetrics).reduce((total, [key, value]) => {
    ...
    // Decision logic
    if (graspScore < 0.60 || solidViolations.length > 0) {
      return {
        decision: 'CREATE_NEW_AGENT',
        confidence: 1 - graspScore,
        rationale: this.generateRationale(graspMetrics, solidMetrics, solidViolations),
    ...
    return {
      ...
      selectedAgent: bestAgent?.name,
      adaptations: this.suggestAdaptations(task, bestAgent || existingAgents[0])
    };
  }

  private assessInformationExpert(task: TaskRequirements, domainExperts: AgentSummary[]): number {
    if (domainExperts.length === 0) return 0;
    
    const bestMatch = domainExperts.reduce((best, agent) => {
      const capabilityMatch = task.capabilities.filter(cap => 
  ...
  private assessCreator(task: TaskRequirements, existingAgents: AgentSummary[]): number {
    const creatorAgents = existingAgents.filter(agent => 
  ...
  private assessController(task: TaskRequirements, existingAgents: AgentSummary[]): number {
    const controllerAgents = existingAgents.filter(agent => 
  ...
  private assessLowCoupling(task: TaskRequirements, existingAgents: AgentSummary[]): number {
    // Higher score means lower coupling (good)
    const domainSpecificAgents = existingAgents.filter(agent => agent.domain === task.domain);
    const couplingScore = domainSpecificAgents.length > 3 ? 0.3 : 0.8;
    
    return couplingScore;
  }

  private assessHighCohesion(task: TaskRequirements, domainExperts: AgentSummary[]): number {
    if (domainExperts.length === 0) return 1.0; // New domain = high cohesion potential
    
    const avgCapabilities = domainExperts.reduce((sum, agent) => 
      sum + agent.capabilities.length, 0) / domainExperts.length;
    
    // Lower number of capabilities per agent = higher cohesion
    return Math.max(0, 1 - (avgCapabilities / 10));
  }

  private assessPolymorphism(task: TaskRequirements, existingAgents: AgentSummary[]): number {
    const polymorphicAgents = existingAgents.filter(agent => 
  ...
  private assessPureDesign(task: TaskRequirements): number {
    ...
    const hasPureElements = task.capabilities.some(cap => 
      domainSpecificKeywords.some(keyword => cap.toLowerCase().includes(keyword))
    );
    
    return hasPureElements ? 0.8 : 0.5;
  }

  private assessIndirection(task: TaskRequirements, existingAgents: AgentSummary[]): number {
    const abstractionAgents = existingAgents.filter(agent => 
      agent.capabilities.includes('abstraction') ||
      agent.capabilities.includes('interface') ||
  ...
  private assessProtectedVariations(task: TaskRequirements, existingAgents: AgentSummary[]): number {
    const stabilityAgents = existingAgents.filter(agent => 
  ...
  private assessSingleResponsibility(task: TaskRequirements, existingAgents: AgentSummary[]): number {
    const domainAgents = existingAgents.filter(agent => agent.domain === task.domain);
    
    if (domainAgents.length === 0) return 1.0; // New domain
    
    const responsibilityOverlap = domainAgents.reduce((overlap, agent) => {
      const commonCaps = task.capabilities.filter(cap => agent.capabilities.includes(cap));
  ...
  private assessOpenClosed(task: TaskRequirements, existingAgents: AgentSummary[]): number {
    const extensibleAgents = existingAgents.filter(agent => 
  ...
  private assessLiskovSubstitution(task: TaskRequirements, existingAgents: AgentSummary[]): number {
    // Assess behavioral compatibility
    const compatibleAgents = existingAgents.filter(agent => 
      agent.domain === task.domain &&
      task.capabilities.every(cap => 
        agent.capabilities.includes(cap) ||
  ...
  private assessInterfaceSegregation(task: TaskRequirements, existingAgents: AgentSummary[]): number {
    const specificAgents = existingAgents.filter(agent => 
  ...
  private assessDependencyInversion(task: TaskRequirements, existingAgents: AgentSummary[]): number {
    const abstractionAgents = existingAgents.filter(agent => 
      agent.capabilities.includes('abstraction') ||
      agent.capabilities.includes('interface') ||
  ...
  private generateRationale(
    ...
    return reasons.length > 0 ? reasons.join('; ') : 'New agent creation recommended for optimal architecture';
  }

  private designAgent(task: TaskRequirements, graspMetrics: GRASPMetrics): AgentSpecification {
    ...
    return {
      ...
      requirements: {
        ...
        performanceTargets: [],
        qualityThresholds: [],
        constraints: task.constraints.map(c => ({ type: 'technical' as const, description: c, impact: 'medium' })),
        dependencies: []
      }
    };
  }

  private selectBestFit(agents: AgentSummary[], task: TaskRequirements): AgentSummary | null {
  ...
  private calculateFitScore(agent: AgentSummary, task: TaskRequirements): number {
    const domainMatch = agent.domain === task.domain ? 0.4 : 0;
    const capabilityMatch = task.capabilities.filter(cap => 
      agent.capabilities.includes(cap)
    ).length / task.capabilities.length * 0.4;
    const performanceScore = agent.performance * 0.2;
    
    return domainMatch + capabilityMatch + performanceScore;
  }

  private suggestAdaptations(task: TaskRequirements, agent: AgentSummary): string[] {
    const adaptations: string[] = [];
    
    const missingCapabilities = task.capabilities.filter(cap => 
      !agent.capabilities.includes(cap)
    );
  ...
  private generateAgentName(task: TaskRequirements): string {
    const domainPart = task.domain.charAt(0).toUpperCase() + task.domain.slice(1);
  ...
  private generateStandards(task: TaskRequirements): any[] {
    const commonStandards = [
      { name: 'ISO/IEC 25010', version: '2011', compliance: ['Quality'], validation: ['Metrics'] },
      { name: 'IEEE SWEBOK', version: 'v3', compliance: ['Engineering'], validation: ['Practices'] }
    ];
  ...
  private generateTools(task: TaskRequirements): any[] {
    return [
      { name: 'Claude Code', type: 'required', usage: 'Primary interface', constraints: [] },
      { name: 'TypeScript', type: 'required', usage: 'Development', constraints: ['Strict mode'] }
    ];
  }
    ...
    return [
      {
        ...
        steps: [
          { id: '1', name: 'Analyze', action: 'analyze-requirements', inputs: ['task'], outputs: ['analysis'] },
          { id: '2', name: 'Execute', action: 'execute-task', inputs: ['analysis'], outputs: ['result'] },
          { id: '3', name: 'Validate', action: 'validate-result', inputs: ['result'], outputs: ['validation'] }
        ],
  ...
  private generateMetrics(task: TaskRequirements): any[] {
    return [
      { name: 'Success Rate', type: 'quality', target: 0.95, unit: 'percentage', measurement: 'completed/total' },
      { name: 'Response Time', type: 'efficiency', target: 30, unit: 'seconds', measurement: 'end-to-end' }
    ];
  }
