...
export type TemplateCategory = 'core' | 'domain_specific' | 'specialized' | 'experimental';
...
export class AgentTemplateSystem {
  ...
  selectTemplate(domain: string, requirements: AgentRequirements): Result<AgentTemplate> {
    try {
      ...
      return Result.fail(`Template selection failed: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  generateAgentFromTemplate(
    ...
    try {
      ...
      return Result.fail(`Agent generation failed: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  registerTemplate(template: AgentTemplate): Result<void> {
    try {
      ...
      if (!validation.isSuccess) {
        return Result.fail(`Template validation failed: ${validation.errorValue()}`);
      }

      this.templates.set(template.id, template);
      return Result.ok(undefined);
    } catch (error) {
      return Result.fail(`Template registration failed: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  updateTemplateMetrics(templateId: string, success: boolean): void {
    ...
    if (template) {
      ...
      const currentSuccessCount = Math.floor(template.metadata.successRate * template.metadata.usageCount);
      const newSuccessCount = success ? currentSuccessCount + 1 : currentSuccessCount;
      template.metadata.successRate = newSuccessCount / template.metadata.usageCount;
      template.metadata.lastModified = new Date();
    }
  }

  private findExactTemplate(domain: string): Result<AgentTemplate> {
    const exactMatch = Array.from(this.templates.values())
      .find(template => template.domain.toLowerCase() === domain.toLowerCase());
    
    return exactMatch ? Result.ok(exactMatch) : Result.fail('No exact template match found');
  }

  private findSimilarTemplates(domain: string): AgentTemplate[] {
    return Array.from(this.templates.values())
      .filter(template => {
        const domainSimilarity = this.calculateDomainSimilarity(template.domain, domain);
  ...
  private selectBestMatch(templates: AgentTemplate[], requirements: AgentRequirements): AgentTemplate {
  ...
  private calculateTemplateScore(template: AgentTemplate, requirements: AgentRequirements): number {
    let score = 0;
    
    // Domain match
    const domainMatch = this.calculateDomainSimilarity(template.domain, requirements.domain);
    score += domainMatch * 0.4;
    
    // Capability coverage
    const capabilityCoverage = this.calculateCapabilityCoverage(template, requirements.capabilities);
  ...
  private calculateCapabilityCoverage(template: AgentTemplate, capabilities: string[]): number {
    const templateCapabilities = this.extractTemplateCapabilities(template);
    const covered = capabilities.filter(cap => 
      templateCapabilities.some(tcap => tcap.toLowerCase().includes(cap.toLowerCase()))
    );
  ...
  private extractTemplateCapabilities(template: AgentTemplate): string[] {
    const responsibilityComponent = template.components.find(c => c.type === 'responsibilities');
    if (!responsibilityComponent) return [];
    
    // Simple extraction - in real implementation, this would be more sophisticated
    const capabilities = responsibilityComponent.content.match(/\b\w+ing\b/g) || [];
    return capabilities;
  }

  private composeTemplate(domain: string, requirements: AgentRequirements): Result<CompositeTemplate> {
    try {
      ...
      return Result.fail(`Template composition failed: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  private selectStandardsModule(domain: string): TemplateComponent[] {
    const standardsMap: Record<string, TemplateComponent[]> = {
      'engineering': [
        this.createStandardComponent('IEEE SWEBOK v3', 'Software engineering best practices'),
        this.createStandardComponent('ISO/IEC 25010', 'Software quality model')
      ],
      'quality': [
        this.createStandardComponent('ISTQB', 'International software testing standards'),
        this.createStandardComponent('ISO/IEC 25010', 'Quality assurance framework')
      ],
      'product': [
        this.createStandardComponent('Pragmatic Marketing', 'Product management framework'),
        this.createStandardComponent('Lean Product', 'Lean development principles')
      ]
    };
  ...
  private selectToolsModule(requirements: AgentRequirements): TemplateComponent[] {
    const tools: TemplateComponent[] = [
      this.createToolComponent('Claude Code', 'required', 'Primary AI interface'),
      this.createToolComponent('TypeScript', 'required', 'Development language')
    ];

    // Add domain-specific tools based on capabilities
    if (requirements.capabilities.includes('testing')) {
      tools.push(this.createToolComponent('Jest', 'required', 'Testing framework'));
    }
    
    if (requirements.capabilities.includes('database')) {
      tools.push(this.createToolComponent('IndexedGraph', 'required', 'Graph database'));
    }
  ...
  private selectPatternsModule(domain: string): TemplateComponent[] {
    const patterns = [
      this.createPatternComponent('Clean Architecture', 'Layered architecture pattern'),
      this.createPatternComponent('SOLID Principles', 'Object-oriented design principles')
    ];

    if (domain === 'engineering') {
      patterns.push(this.createPatternComponent('Repository Pattern', 'Data access abstraction'));
      patterns.push(this.createPatternComponent('Use Case Pattern', 'Business logic organization'));
    }
  ...
  private selectProtocolsModule(): TemplateComponent[] {
    return [
      this.createProtocolComponent('Task Assignment', 'YAML', 'Standard task routing protocol'),
      this.createProtocolComponent('Status Update', 'JSON', 'Progress reporting protocol'),
      this.createProtocolComponent('Completion Report', 'Markdown', 'Task completion documentation')
    ];
  }

  private templateFromComposition(composition: CompositeTemplate): AgentTemplate {
  ...
  private generateFromBase(domain: string, requirements: AgentRequirements): AgentTemplate {
    return {
  ...
  private generateStandardComponents(domain: string, requirements: AgentRequirements): TemplateComponent[] {
    return [
      {
        name: 'header',
        type: 'header',
  ...
  private generateStandardVariables(requirements: AgentRequirements): TemplateVariable[] {
    return [
      {
        name: 'name',
        type: 'string',
  ...
  private renderTemplate(template: AgentTemplate, variables: Record<string, any>): string {
    let content = this.assembleTemplate(template);
    
    // Replace variables
    Object.entries(variables).forEach(([key, value]) => {
      const placeholder = new RegExp(`\\{\\{${key}\\}\\}`, 'g');
      const replacement = Array.isArray(value) ? this.formatArray(value) : String(value);
  ...
  private assembleTemplate(template: AgentTemplate): string {
    const sortedComponents = template.components.sort((a, b) => {
      const order: ComponentType[] = [
        'header', 'responsibilities', 'standards', 'tools', 
        'protocols', 'workflows', 'metrics', 'best_practices', 'custom'
      ];
...
{{/each}}`;
  ...
  private createStandardComponent(name: string, description: string): TemplateComponent {
    return {
  ...
  private createToolComponent(name: string, type: string, description: string): TemplateComponent {
    return {
      name: name.toLowerCase().replace(/\s+/g, '-'),
      type: 'tools',
  ...
  private createPatternComponent(name: string, description: string): TemplateComponent {
    return {
      name: name.toLowerCase().replace(/\s+/g, '-'),
      type: 'custom',
      required: false,
  ...
  private createProtocolComponent(name: string, format: string, description: string): TemplateComponent {
    return {
      name: name.toLowerCase().replace(/\s+/g, '-'),
      type: 'protocols',
      required: false,
