...
export class GetClassHierarchyUseCase implements UseCase<GetClassHierarchyRequest, GetClassHierarchyResponse> {
  ...
  async execute(request: GetClassHierarchyRequest): Promise<Result<GetClassHierarchyResponse>> {
    try {
      const { className, maxDepth = GetClassHierarchyUseCase.DEFAULT_MAX_DEPTH } = request;
      ...
      const buildHierarchy = async (currentClass: string, depth: number): Promise<void> => {
   ...
   */
  private async findClassDefinitionFile(className: string): Promise<any | null> {
