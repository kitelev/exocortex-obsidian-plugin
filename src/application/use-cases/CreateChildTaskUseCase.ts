...
export class CreateChildTaskUseCase {
  ...
  async execute(request: CreateChildTaskRequest): Promise<CreateChildTaskResponse> {
    try {
      ...
      if (!projectClassName.equals(ClassName.create('ems__Project').getValue())) {
        return {
          success: false,
  ...
  private generateTaskProperties(parentProject: Asset, taskId: AssetId): Record<string, any> {
    return {
