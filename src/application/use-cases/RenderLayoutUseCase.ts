...
import { ILayoutRenderer, RenderContext, LayoutRenderResult } from '../../domain/ports/ILayoutRenderer';
...
export class RenderLayoutUseCase {
    ...
    async execute(request: RenderLayoutRequest): Promise<Result<LayoutRenderResult>> {
        try {
            // Get file metadata
            const metadataResult = await this.fileSystemPort.getFileMetadata(request.assetPath);
            if (metadataResult.isFailure) {
                return Result.fail(`Failed to read file metadata: ${metadataResult.error}`);
            }
            ...
            if (layout) {
                ...
                return Result.fail('No layout found for class and default rendering disabled');
            }
        } catch (error) {
    ...
    private async getLayoutForClass(className: string): Promise<ClassLayout | null> {
        const classNameResult = ClassName.create(className);
        if (classNameResult.isFailure) {
            return null;
        }

        const layouts = await this.layoutRepository.findEnabledByClass(classNameResult.getValue());
