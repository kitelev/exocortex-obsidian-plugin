...
export class ExoAgent {
    ...
    private extractFilters(query: string): Record<string, any> {
        ...
        if (query.toLowerCase().includes('pending') || query.toLowerCase().includes('todo')) {
            filters.status = 'pending';
        }
        
        if (query.toLowerCase().includes('completed') || query.toLowerCase().includes('done')) {
            filters.status = 'completed';
        }
        
        if (query.toLowerCase().includes('high priority') || query.toLowerCase().includes('urgent')) {
