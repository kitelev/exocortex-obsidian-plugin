...
export class LayoutCoordinator implements ILayoutCoordinator {
  ...
  async coordinateLayout(
    ...
    try {
      ...
      const attachResult = this.domRenderer.attachToContainer(
        context.containerId,
        layoutInfo,
      );
      if (attachResult.isFailure) {
        return Result.fail(
          `Failed to attach layout info: ${attachResult.error}`,
        );
      }
  ...
  private async renderBlock(
    ...
    try {
      ...
      const blockContainer = this.containerFactory.createBlockContainer(
        block.type,
        block.id,
      );
      ...
      if (block.title) {
        const header = this.containerFactory.createHeaderContainer(
          block.title,
          block.isCollapsible,
        );
      ...
      if (attachResult.isFailure) {
        return Result.fail(
          `Failed to attach block container: ${attachResult.error}`,
        );
      }
      ...
      return await this.blockRenderingService.renderBlock(
        block,
        contentContext,
      );
    } catch (error) {
