...
export class BlockRenderingService {
    ...
    registerStrategy(strategy: IBlockRenderStrategy): void {
        ...
        const blockTypes = ['query', 'properties', 'backlinks', 'children-efforts', 'narrower', 'buttons', 'custom'];
    ...
    async renderBlock(
        ...
        try {
            ...
            if (!strategy) {
                return Result.fail(`No rendering strategy found for block type: ${block.type}`);
            }
            ...
            return Result.fail(`Unexpected error rendering block ${block.id}: ${error}`);
        }
    }
