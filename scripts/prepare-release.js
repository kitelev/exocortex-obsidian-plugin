...
class ReleasePreparation {
  ...
  validateCurrentBranch() {
    ...
    if (currentBranch !== "main" && currentBranch !== "master") {
      this.warn(
        `Currently on branch '${currentBranch}'. Consider releasing from main/master branch.`,
      );
    } else {
  ...
  validatePlugin() {
    ...
    if (!validateResult.success) {
      this.error(
        "Plugin validation failed. Fix validation errors before release.",
      );
      return false;
    }
  ...
  updateChangelog() {
    ...
    const changelogResult = this.exec(
  ...
  checkVersions() {
    ...
    if (packageVersion && manifestVersion !== packageVersion) {
      this.error(
        `Version mismatch: manifest.json (${manifestVersion}) vs package.json (${packageVersion})`,
      );
      return false;
    }
    ...
    try {
      ...
      if (
        tags.includes(manifestVersion) ||
        tags.includes(`v${manifestVersion}`)
      ) {
        this.warn(
          `Version ${manifestVersion} already has a git tag. Consider bumping the version.`,
        );
      }
    } catch (error) {
  ...
  generateReleaseArtifacts() {
    ...
    if (mainSizeMB > 5) {
      this.warn(
        `main.js is quite large (${mainSizeMB.toFixed(2)}MB). Consider optimization.`,
      );
    }
