...
class ChangelogGenerator {
    ...
    getCommitsBetween(from, to = 'HEAD') {
        try {
            ...
            const commits = execSync(`git log ${range} --pretty=format:"%H|%s|%an|%ad" --date=short`, { encoding: 'utf8' }).trim();
    ...
    categorizeCommit(commitSubject) {
        ...
        for (const [keyword, category] of Object.entries(this.changeTypes)) {
            if (subject.startsWith(keyword + ' ') || subject.startsWith(keyword + ':')) {
                return { category, description: commitSubject };
            }
        }
...
`;
    ...
    generateFullChangelog() {
        ...
        if (tags.length === 0) {
            ...
            const section = this.generateChangelogSection(currentVersion, allCommits, null);
            if (section) {
                changelog += section;
            }
        } else {
            ...
            if (unreleasedCommits.length > 0) {
                const unreleasedSection = this.generateChangelogSection('Unreleased', unreleasedCommits, null);
            ...
            for (let i = 0; i < tags.length; i++) {
                ...
                const section = this.generateChangelogSection(currentTag, commits, null);
                if (section) {
                    changelog += section;
                }
            }
    ...
    generateIncremental() {
        ...
        const newSection = this.generateChangelogSection(currentVersion, newCommits, null);
        if (!newSection) {
