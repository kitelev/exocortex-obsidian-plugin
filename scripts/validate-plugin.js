...
class PluginValidator {
    ...
    validateManifest() {
        ...
        if (manifest.id) {
            if (!/^[a-z0-9-]+$/.test(manifest.id)) {
                this.error('Plugin ID must only contain lowercase letters, numbers, and hyphens');
            }
        ...
        if (manifest.version) {
            if (!/^\d+\.\d+\.\d+(-[a-zA-Z0-9-.]+)?$/.test(manifest.version)) {
                this.error('Plugin version must follow semantic versioning (e.g., 1.0.0)');
            }
        }
        ...
        if (manifest.description && manifest.description.length > 250) {
            this.warn('Plugin description should be under 250 characters for better display');
        }
    ...
    validateVersionsFile() {
        ...
        for (const [version, minAppVersion] of Object.entries(versions)) {
            ...
            if (!/^\d+\.\d+\.\d+$/.test(minAppVersion)) {
                this.error(`Invalid minAppVersion format in versions.json: ${minAppVersion} for version ${version}`);
            }
        }
    ...
    validateMainFile() {
        ...
        if (fileSizeInMB > 2) {
            this.warn(`main.js is quite large (${fileSizeInMB.toFixed(2)}MB). Consider code splitting.`);
        }
    ...
    validateVersionConsistency() {
        ...
        if (packageVersion && manifestVersion && packageVersion !== manifestVersion) {
            this.warn(`Version mismatch: package.json (${packageVersion}) vs manifest.json (${manifestVersion})`);
    ...
    validateDirectoryStructure() {
        ...
        for (const dir of expectedDirs) {
            if (fs.existsSync(dir) && fs.statSync(dir).isDirectory()) {
                ...
                this.info(`${dir}/ directory not found (recommended for organized projects)`);
            }
        }
    ...
    validatePackageJson() {
        ...
        if (!devDeps.obsidian) {
            this.warn('obsidian package not found in devDependencies (recommended for TypeScript support)');
        }
        ...
        if (packageJson.engines && packageJson.engines.node) {
            ...
            if (!nodeVersion.includes('18') && !nodeVersion.includes('20')) {
                this.warn('Consider specifying Node.js 18+ in engines for better CI compatibility');
            }
        }
    ...
    run() {
        ...
        if (this.errors.length === 0) {
            console.log('🎉 Plugin validation passed! Your plugin is ready for release.');
            return true;
        } else {
