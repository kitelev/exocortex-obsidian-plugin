    ...
    const element = {
      ...
      appendChild: function (child) {
        ...
        if (child.className === "exocortex-sparql-container") {
          console.log(
            "✅ Container created with class: exocortex-sparql-container",
          );
        }
      },
      empty: function () {
...
try {
  ...
  if (plugin.processor) {
    ...
    // Check for container
    const container = mockEl.children.find(
      (c) => c.className === "exocortex-sparql-container",
    );
    if (container) {
      ...
      if (title && title.textContent === "SPARQL Query Results") {
        ...
        console.log(
          `❌ Title incorrect: "${title?.textContent || "not found"}"`,
        );
      }
      ...
      console.log(
        "Children:",
        mockEl.children.map((c) => c.className),
      );
    }
    ...
    const container2 = mockEl2.children.find(
      (c) => c.className === "exocortex-sparql-container",
    );
    if (container2) {
      const error = container2.children.find(
        (c) => c.className === "sparql-error",
      );
      if (error) {
