...
export const config = {
  ...
  before: async function (capabilities, specs, browser) {
        ...
        await (this as any).waitUntil(
          async () => {
            try {
              ...
              console.warn(
                "⏳ Waiting for Obsidian to be ready...",
                error.message,
              );
              return false;
            }
    ...
    browser.addCommand(
      ...
      async function (testName: string) {
  ...
  beforeTest: async function (test, context) {
    const testName = `${test.parent}.${test.title}`.replace(
      /[^a-zA-Z0-9]/g,
      "_",
    );
  ...
  afterTest: async function (test, context, result) {
    ...
    if (!result.passed) {
      ...
      if ((browser as any).testInfo?.name) {
        await (browser as any).takeScreenshotOnFailure(
          (browser as any).testInfo.name,
        );
      }
    } else {
