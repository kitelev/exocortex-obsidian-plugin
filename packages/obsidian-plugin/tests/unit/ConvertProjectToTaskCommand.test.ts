...
describe("ConvertProjectToTaskCommand", () => {
  ...
  beforeEach(() => {
    ...
    command = new ConvertProjectToTaskCommand(mockConversionService);
  });
  ...
  describe("checkCallback", () => {
    ...
    it("should execute conversion when checking is false", async () => {
      ...
      mockConversionService.convertProjectToTask.mockResolvedValue(
        mockFile as any,
      );
      ...
      expect(result).toBe(true);
      expect(mockConversionService.convertProjectToTask).toHaveBeenCalledWith(
        mockFile,
      );
    });
