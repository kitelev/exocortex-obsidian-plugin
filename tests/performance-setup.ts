...
export const measurePerformance = <T>(
  ...
  result: T;
  times: number[];
  stats: { avg: number; min: number; max: number; stdDev: number };
} => {
  // Warmup phase
  let lastResult: T;
...
export const comparePerformance = <T1, T2>(
  ...
  const faster =
    result1.stats.avg < result2.stats.avg ? "operation1" : "operation2";
