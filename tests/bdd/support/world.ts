...
export class BDDWorld implements IBDDWorld {
   ...
   */
  recordPerformance(
    metric: keyof BDDWorld["performanceMetrics"],
    value: number,
  ): void {
  ...
  private logPerformanceSummary(): void {
    ...
    console.log(
      `   Execution Time: ${this.performanceMetrics.executionTime}ms`,
    );
   ...
   */
  assertPerformance(
    requirements: Partial<BDDWorld["performanceMetrics"]>,
  ): void {
    if (
      requirements.executionTime &&
      this.performanceMetrics.executionTime > requirements.executionTime
    ) {
      throw new Error(
        `Execution time ${this.performanceMetrics.executionTime}ms exceeds limit ${requirements.executionTime}ms`,
      );
    }

    if (
      requirements.memoryUsage &&
      this.performanceMetrics.memoryUsage > requirements.memoryUsage
    ) {
      throw new Error(
        `Memory usage ${this.performanceMetrics.memoryUsage}MB exceeds limit ${requirements.memoryUsage}MB`,
      );
    }
  }
   ...
   */
  createFeatureContext<T extends Record<string, any>>(
    featureName: string,
    initialState: T,
  ): T & { cleanup: () => void } {
