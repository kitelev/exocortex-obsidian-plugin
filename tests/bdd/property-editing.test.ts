...
const feature = loadFeature(path.join(__dirname, 'features/property-editing.feature'));
...
defineFeature(feature, test => {
    ...
    and('the changes should be atomic across all assets', () => {
      ...
      if (assets.length > 1) {
        ...
        assets.forEach((asset: any) => {
          expect(asset.getUpdatedAt().getTime()).toBeCloseTo(firstTimestamp.getTime(), -2);
        });
      }
    });
  ...
  test('Property validation with error handling', ({ given, when, then, and }) => {
  ...
  test('Performance validation for property editing', ({ given, when, then }) => {
