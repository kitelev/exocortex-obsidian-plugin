...
const feature = loadFeature(
  path.join(__dirname, "features/asset-management.feature"),
);
...
defineFeature(feature, (test) => {
  ...
  test("Creating a new asset with valid properties", ({
    given,
    when,
    then,
    and,
    ...
    given("the class hierarchy is loaded", () => {
      ...
      expect(["ems__Project", "ems__Task", "ems__Area"]).toContain(
        "ems__Project",
      );
    });
    ...
    and("the asset should be indexed in the graph", () => {
      ...
      const typeTriples = world.graph.match({
        subject: assetIRI,
        predicate: "rdf:type",
      });
  ...
  test("Creating an asset with invalid class fails gracefully", ({
    given,
    when,
    then,
    and,
  ...
  test("Managing multiple assets efficiently", ({ given, when, then, and }) => {
    ...
    then(
      /^all assets should be created within (\d+) seconds$/,
      (maxSeconds) => {
    ...
    and(/^the memory usage should remain under (\d+)MB$/, (maxMemoryMB) => {
      const memoryCheck = world.performanceMonitor.checkMemoryLeak(
        parseInt(maxMemoryMB),
      );
  ...
  test("Asset creation with security validation", ({
    given,
    when,
    then,
    and,
    ...
    when("I attempt to create the asset", async () => {
      ...
      const validation = world.securityValidator.validateInput(
        config.name,
        "asset_name",
      );
