...
export class SecurityValidator {
  ...
  validateSPARQLQuery(query: string): SPARQLSecurityResult {
    ...
    const dangerousOperations = [
      'delete', 'insert', 'drop', 'clear', 'create',
      'load', 'copy', 'move', 'add'
    ];
    ...
    dangerousPaths.forEach(dangerousPath => {
      if (path.toLowerCase().includes(dangerousPath.toLowerCase())) {
        issues.push({
          ...
          recommendation: 'Restrict file access to application directories only'
        });
      }
    });
  ...
  private sanitizeSPARQLQuery(query: string, issues: SecurityIssue[]): string {
    ...
    sanitized = sanitized.replace(/delete\s+where/gi, '# REMOVED: DELETE WHERE');
  ...
  generateSecurityReport(): SecurityReport {
    return {
      ...
      summary: {
        ...
        criticalIssues: this.securityWarnings.filter(w => w.severity === 'critical').length,
        highIssues: this.securityWarnings.filter(w => w.severity === 'high').length,
        mediumIssues: this.securityWarnings.filter(w => w.severity === 'medium').length,
        lowIssues: this.securityWarnings.filter(w => w.severity === 'low').length
      },
      timestamp: new Date()
    };
  }
