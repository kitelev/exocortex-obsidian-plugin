...
beforeAll(async () => {
  ...
  console.log(`📋 Loaded ${global.bddContext.featureFiles.length} feature files`);
...
afterAll(async () => {
  ...
  const performanceReport = global.bddContext.performanceMonitor.generateReport();
  console.log('📊 Performance Summary:', performanceReport);
  
  // Generate security report
  const securityReport = await global.bddContext.securityValidator.generateReport();
...
expect.extend({
  toBeValidAsset(received: any) {
    if (!received || typeof received !== 'object') {
      return {
        message: () => `Expected valid asset object, received ${typeof received}`,
        pass: false,
      };
    }
    
    const hasRequiredProperties = received.id && received.className && received.title;
  ...
  toHaveValidFrontmatter(received: string) {
    const hasFrontmatter = received.startsWith('---') && received.includes('---\n');
  ...
  toHaveSecurityViolations(received: any) {
    const violations = global.bddContext.securityValidator.checkForViolations(received);
