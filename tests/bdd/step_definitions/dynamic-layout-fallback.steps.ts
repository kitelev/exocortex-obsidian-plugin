...
Given(
  ...
  function (assetName: string, className: string) {
...
When(
  "DynamicLayout is rendered for {string}",
  async function (assetName: string) {
    ...
    await dynamicRenderer.render(
      mockDv as any,
      mockFile,
      testAsset.frontmatter,
      renderedContent,
    );
  },
);
...
Then("I should see the message {string}", function (expectedMessage: string) {
  const actualMessage = expectedMessage.replace(
    "[[CustomClass]]",
    testAsset.frontmatter["exo__Instance_class"][0],
  );
  const content = renderedContent.textContent || "";
  expect(content).to.include(
    actualMessage.replace("[[CustomClass]]", "[[CustomClass]]"),
  );
});

Then(
  "the UniversalLayout content should be displayed below the message",
  function () {
    const elements = Array.from(renderedContent.children);
    expect(elements.length).to.be.greaterThan(1);

    const hasPropertiesSection = elements.some(
      (el) =>
        el.textContent?.includes("Properties") ||
        el.textContent?.includes("Relations"),
    );
...
Then(
  "the fallback message should contain {string}",
...
Then("not {string} without brackets", function (unwantedFormat: string) {
  ...
  if (messageStart !== -1) {
    const messageEnd = content.indexOf(
...
Then("the layout should match UniversalLayout's standard output", function () {
  const hasStandardSections =
    renderedContent.querySelector("h2") !== null ||
...
Then(
  "each should show its own class name in the fallback message",
  function () {
