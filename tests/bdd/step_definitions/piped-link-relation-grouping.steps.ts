...
class TestRenderer extends BaseAssetRelationsRenderer {
  async render(source: string, container: HTMLElement, ctx: any): Promise<void> {
    // Not used in tests
  }
...
Given('I have an asset {string} with class {string}', function (name: string, className: string) {
  const asset = {
    path: `${name}.md`,
    basename: name,
    frontmatter: {
...
Given('I have an asset {string} with property {string} containing {string}', 
...
Given('I have assets with different link formats to {string}:', 
...
Given('the {string} asset is at path {string}', function (assetName: string, path: string) {
...
Given('I have an asset {string} with property {string} containing:', 
...
Given('I have an asset {string} with body text containing {string}', 
...
Given('{string} is at path {string}', function (assetName: string, path: string) {
...
When('UniversalLayout or DynamicLayout renders relations for {string}', 
  async function (targetName: string) {
    const targetFile = testAssets.get(`${targetName}.md`) || 
    ...
    if (targetFile) {
      ...
      groupedRelations = renderer.testGroupRelationsByProperty(collectedRelations);
    }
  }
);

When('relations are collected for {string}', async function (targetName: string) {
  await this.When(`UniversalLayout or DynamicLayout renders relations for "${targetName}"`);
});

Then('both {string} and {string} should appear in the {string} group', 
...
Then('all four assets should be in the {string} group', function (groupName: string) {
  const group = groupedRelations.get(groupName);
  expect(group).to.exist;
...
Then('{string} should appear in the {string} group', function (assetName: string, groupName: string) {
  const group = groupedRelations.get(groupName);
  expect(group).to.exist;
...
Then('{string} should appear once in the {string} group', 
...
Then('the property should be correctly identified for all link formats', function () {
...
Then('{string} should appear in {string}', function (assetName: string, groupName: string) {
  const group = groupedRelations.get(groupName);
  expect(group).to.exist;
...
Then('they should not be grouped together', function () {
  ...
  const typedGroups = Array.from(groupedRelations.entries())
    .filter(([key]) => key !== 'Untyped Relations');
