...
Before({ tags: '@query-execution' }, async function() {
  ...
  context = {
    container,
    executeQueryUseCase: container.resolve<ExecuteQueryBlockUseCase>('ExecuteQueryBlockUseCase'),
...
When('I execute the following SPARQL query:', async function(queryString: string) {
...
Then('the constructed triples should contain:', function(dataTable: DataTable) {
  ...
  if (context.lastQuery.includes('CONSTRUCT')) {
    ...
    expectedTriples.forEach(expected => {
      const found = triples.some((triple: any) => {
        return triple.subject === expected.subject &&
...
When('I execute an invalid SPARQL query:', async function(invalidQuery: string) {
...
When('I request pagination with {int} results per page', function(pageSize: number) {
  context.paginationConfig.pageSize = pageSize;
});

Then('the first page should contain {int} results', function(expectedResults: number) {
  expect(context.queryResults).to.not.be.null;
...
Then('the next {int} results should be returned', function(expectedResults: number) {
  const bindings = context.queryResults.bindings || [];
  const startIndex = (context.paginationConfig.page - 1) * context.paginationConfig.pageSize;
  const pageResults = bindings.slice(startIndex, startIndex + context.paginationConfig.pageSize);
  expect(pageResults.length).to.equal(expectedResults);
});
...
Given('the query timeout is set to {int} seconds', function(timeoutSeconds: number) {
...
When('I start {int} concurrent SELECT queries', async function(queryCount: number) {
