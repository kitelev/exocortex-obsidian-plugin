...
Before({ tags: "@asset-management" }, async function () {
  ...
  context = {
    ...
    propertyEditingUseCase: container.resolve<PropertyEditingUseCase>(
...
Given("the class hierarchy is loaded", function () {
  ...
  expect(validClasses).to.include.members([
    "ems__Project",
    "ems__Task",
    "ems__Area",
  ]);
  ...
  try {
    ...
    const assetResult = Asset.create({
      ...
      className: ClassName.create(
        context.assetConfiguration.className,
      ).getValue()!,
      properties: new Map(),
    });
...
Then("the asset should have the correct properties", function () {
  ...
  expect(asset.getClassName().getValue()).to.equal(
    context.assetConfiguration.className,
  );
});
...
Then(
  "all assets should be created within {int} seconds",
...
Then(
  "the memory usage should remain under {int}MB",
  function (maxMemoryMB: number) {
...
// Security scenarios
Given(
  "I have asset data with potential security risks",
...
When(
  "I search for assets by class {string}",
  async function (className: string) {
...
When("I lookup an asset by name {string}", async function (assetName: string) {
  const asset = context.createdAssets.find((a) => a.getTitle() === assetName);
...
Then("the system should handle the failure gracefully", function () {
  ...
  expect(
    context.lastError || (context.lastResult && !context.lastResult.isSuccess),
  ).to.be.true;
});
...
Then("the user should receive an appropriate error message", function () {
  ...
  expect(errorMessage).to.match(
    /temporarily unavailable|read-only|permission/i,
  );
});
...
Then(
  "the asset should be created once the vault is available",
  async function () {
...
When(
  "I link asset {string} to asset {string}",
  async function (sourceAsset: string, targetAsset: string) {
    const source = context.createdAssets.find(
      (a) => a.getTitle() === sourceAsset,
    );
    const target = context.createdAssets.find(
      (a) => a.getTitle() === targetAsset,
    );

    expect(source).to.not.be.null;
    expect(target).to.not.be.null;

    const linkResult = source!.setProperty(
      "linkedTo",
...
Then("the parent-child relationship should be established", function () {
  ...
  expect(parentId).to.equal(
    context.assetConfiguration.parent.getId().toString(),
  );
});
...
When("I attempt to update with invalid data", async function () {
  try {
    ...
    const updateResult = asset.updateProperties(
      context.assetConfiguration.invalidProperties,
    );
...
Then("the original value should be preserved", function () {
  ...
  expect(asset.getPropertyValue(propertyName)).to.not.equal(
    context.assetConfiguration.invalidValue,
  );
});
...
Then(
  "the persistence should complete within {int}ms",
...
When(
  "User A changes {string} to {string}",
  function (property: string, valueA: string) {
    context.assetConfiguration.userAChanges = { [property]: valueA };
  },
);

When(
  "User B simultaneously changes {string} to {string}",
  function (property: string, valueB: string) {
...
When("both users save their changes", async function () {
  ...
  // User A update
  const resultA = asset.updateProperties(
    context.assetConfiguration.userAChanges,
  );

  // User B update (should detect conflict)
  const resultB = asset.updateProperties(
    context.assetConfiguration.userBChanges,
  );
...
Then("I should see only properties common to all assets", function () {
  ...
  expect(context.assetConfiguration.commonProperties).to.include.members([
    "priority",
    "status",
  ]);
});
...
Then("all selected assets should be updated", function () {
  ...
  expect(context.lastResult!.isSuccess).to.be.true;
  const result = context.lastResult!.getValue();
  expect(result.updatedAssets).to.equal(
    context.assetConfiguration.batchAssets.length,
  );
});
...
Then("the changes should be atomic across all assets", function () {
  ...
  if (batchAssets.length > 1) {
    ...
    batchAssets.forEach((asset: Asset) => {
      expect(asset.getUpdatedAt().getTime()).to.be.closeTo(
...
Given("the asset has the following properties:", function (dataTable) {
  ...
  if (context.currentAsset) {
    for (const [key, value] of Object.entries(properties)) {
      if (key !== "class") {
        // Skip class as it's handled differently
...
Then(
  "the property should be updated to {string}",
  function (expectedValue: string) {
    const propertyName = context.assetConfiguration.clickedProperty;
    expect(context.currentAsset!.getPropertyValue(propertyName)).to.equal(
      expectedValue,
    );
  },
);
...
function hasFile(adapter: any, filename: string): boolean {
  try {
    return adapter.hasFile
      ? adapter.hasFile(filename)
      : adapter.files?.has(filename) || false;
  } catch {
    return false;
  }
}
 ...
 */
function validateAssetStructure(
  asset: Asset,
...
function createMockFrontmatter(asset: Asset): string {
  ...
  lines.push("");
  lines.push(
    asset.getPropertyValue("description") || "No description provided.",
  );
...
class PerformanceMeasurer {
  ...
  start(operation: string): void {
    ...
    const startTime =
      this.measurements.get(operation + "_start") || this.startTime;
...
function ensureVaultAdapterInterface(adapter: any): void {
  ...
  if (adapter && typeof adapter.createFile !== "function") {
    adapter.createFile = async function (
      path: string,
      content: string,
    ): Promise<void> {
