...
When(
  "I edit {string} from {string} to {string}",
  async function (property: string, oldValue: string, newValue: string) {
    try {
      const result = await context.propertyEditingUseCase.execute({
        assetId: context.currentAsset.getId().toString(),
...
Then(
  "{string} should revert to {string}",
  function (property: string, expectedValue: string) {
    const lastChange = context.changeHistory[context.changeHistory.length - 1];
...
Then(
  ...
  function (property: string, expectedValue: string) {
    ...
    const propertyChanges = context.changeHistory.filter(
...
Given(
  "I have a custom property type {string}",
...
Then(
  "the appropriate editor for {string} should appear",
  function (propertyType: string) {
...
When("both users save their changes", function () {
  const userActions: UserAction[] =
    context.pendingChanges.get("userActions") || [];
...
Then("the property should be announced clearly", function () {
  expect(context.pendingChanges.get("navigationTarget")).to.equal(
    "propertyField",
  );
  expect(context.pendingChanges.get("assistiveTechnology")).to.equal(
    "screenReader",
  );
});
...
Then(
  "{string} should become {string} again",
  function (property: string, expectedValue: string) {
    const lastChange = context.changeHistory[context.changeHistory.length - 1];
    if (lastChange.property === property) {
      expect(lastChange.newValue).to.equal(expectedValue);
...
Then("a length validation error should be shown", function () {
  expect(
    context.validationErrors.some(
      (error) => error.includes("length") || error.includes("maximum"),
    ),
  ).to.be.true;
});
