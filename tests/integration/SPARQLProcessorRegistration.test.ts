...
describe("SPARQL Processor Registration", () => {
  ...
  beforeEach(() => {
    ...
    plugin.registerMarkdownCodeBlockProcessor = jest.fn(
      (language: string, handler: any) => {
        if (registeredProcessors.has(language)) {
          throw new Error(
            `Code block postprocessor for language ${language} is already registered`,
          );
        }
  ...
  describe("Plugin Loading", () => {
    it("should register SPARQL processor only once on first load", async () => {
      ...
      expect(plugin.registerMarkdownCodeBlockProcessor).toHaveBeenCalledTimes(
        2,
      );
    ...
    it("should handle duplicate registration gracefully", async () => {
      ...
      plugin2.registerMarkdownCodeBlockProcessor = jest.fn(
        (language: string, handler: any) => {
          if (registeredProcessors.has(language)) {
            throw new Error(
              `Code block postprocessor for language ${language} is already registered`,
    ...
    it("should handle reload scenario correctly", async () => {
      ...
      expect(plugin2.registerMarkdownCodeBlockProcessor).toHaveBeenCalledTimes(
        2,
      );
