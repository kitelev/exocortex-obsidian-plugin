...
describe("SPARQLEngine Caching Integration", () => {
  ...
  beforeEach(() => {
    ...
    graph.add(
      new Triple(
        new IRI("file://test1"),
        new IRI("dc:title"),
        Literal.string("Test Document 1"),
      ),
    );
    graph.add(
      new Triple(
    ...
    graph.add(
      new Triple(
        new IRI("file://test2"),
        new IRI("dc:title"),
        Literal.string("Test Document 2"),
      ),
    );
    graph.add(
      new Triple(
    ...
    graph.add(
      new Triple(
        new IRI("file://test3"),
        new IRI("dc:title"),
        Literal.string("Test Document 3"),
      ),
    );
    graph.add(
      new Triple(
  ...
  describe("SELECT query caching", () => {
    ...
    it("should handle query normalization for caching", () => {
      ...
      const query2 =
        "  SELECT   ?s   ?title   WHERE   {   ?s   dc:title   ?title   }  ";
      const query3 = "SELECT\n?s\n?title\nWHERE\n{\n?s\ndc:title\n?title\n}";
  ...
  describe("CONSTRUCT query caching", () => {
    it("should cache CONSTRUCT query results", () => {
      const query =
        "CONSTRUCT { ?s ex:hasType ?type } WHERE { ?s rdf:type ?type }";
    ...
    it("should not add cached triples to graph again", () => {
      const query =
        "CONSTRUCT { ?s ex:derived_type ?type } WHERE { ?s rdf:type ?type }";
    ...
    it("should cache CONSTRUCT queries with LIMIT", () => {
      const query =
        "CONSTRUCT { ?s ex:limited_type ?type } WHERE { ?s rdf:type ?type } LIMIT 2";
    ...
    it("should handle empty CONSTRUCT results", () => {
      const query =
        "CONSTRUCT { ?s ex:empty_construct ?value } WHERE { ?s ex:nonexistent ?value }";
