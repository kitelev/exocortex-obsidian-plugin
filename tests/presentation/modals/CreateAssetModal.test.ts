...
beforeAll(() => {
  HTMLElement.prototype.createEl = jest
    .fn()
  ...
  HTMLElement.prototype.createDiv = jest
    .fn()
...
describe("CreateAssetModal", () => {
  ...
  describe("Class Field", () => {
    ...
    test("should update properties when class changes", async () => {
      const containerEl = document.createElement("div");
      const updatePropertiesForClassSpy = jest.spyOn(
        modal as any,
        "updatePropertiesForClass",
      );
  ...
  describe("Property Value Handling", () => {
    test("should handle array values correctly", () => {
      ...
      (modal as any).createArrayField = jest
        .fn()
        .mockImplementation((setting, prop) => {
          // Simulate adding comma-separated values
          (modal as any).propertyValues.set(prop.name, [
            "tag1",
            "tag2",
            "tag3",
          ]);
    ...
    test("should handle number parsing correctly", () => {
      ...
      (modal as any).createNumberField = jest
        .fn()
        .mockImplementation((setting, prop) => {
