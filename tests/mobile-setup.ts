    ...
    global.requestIdleCallback = jest.fn(
      (callback: IdleRequestCallback, options?: IdleRequestOptions) => {
        const id = setTimeout(() => {
          callback({
            timeRemaining: () =>
              options?.timeout ? Math.max(0, options.timeout - 50) : 5,
            didTimeout: false,
          });
        }, 0);
        return id;
...
export class MobileTestEnvironment {
  ...
  static simulateMemoryPressure(
    pressureLevel: "low" | "moderate" | "critical" = "moderate",
  ) {
    const memoryLevels = {
      low: {
  ...
  static createGestureSequence(element: Element) {
    return {
      ...
      async tap(x: number, y: number) {
        const touchStart = this.createTouchEvent(
          "touchstart",
          [{ x, y }],
          element,
        );
      ...
      async pinch(
        ...
        const touchMove = this.createTouchEvent(
          "touchmove",
          [
