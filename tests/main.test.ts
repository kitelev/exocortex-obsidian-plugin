    ...
    app = {
      vault: {
        getMarkdownFiles: jest
          .fn()
          .mockReturnValue([{ basename: "test-file", path: "test-file.md" }]),
...
# Test File`),
  ...
  describe("DOM Processing", () => {
    test("should register markdown code block processors", async () => {
      ...
      const registerProcessorSpy = jest
        .spyOn(plugin, "registerMarkdownCodeBlockProcessor" as any)
        .mockImplementation(() => {});
      ...
      expect(registerProcessorSpy).toHaveBeenCalledWith(
        "sparql",
        expect.any(Function),
      );
      expect(registerProcessorSpy).toHaveBeenCalledWith(
