...
export class FakeLayoutCoordinator implements ILayoutCoordinator {
    private rendered: Array<{ layout: ClassLayout | null; context: RenderContext }> = [];
    ...
    public getRenderedLayouts(): Array<{ layout: ClassLayout | null; context: RenderContext }> {
        return [...this.rendered];
    }

    public getLastRenderContext(): RenderContext | null {
        return this.rendered.length > 0 ? this.rendered[this.rendered.length - 1].context : null;
    }
    ...
    async coordinateLayout(
        ...
        const blockResults: BlockRenderResult[] = layout.getVisibleBlocks().map(block => ({
            blockId: block.id,
            renderTime: 10,
            success: true
        }));
