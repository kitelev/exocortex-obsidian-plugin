...
test.describe('Create Child Task Button UI Tests', () => {
    ...
    test('Button should be visible in ems__Project asset view', async () => {
        ...
        await page.goto(`obsidian://open?vault=${encodeURIComponent(vaultPath)}&file=${encodeURIComponent(projectFilePath)}`);
        await page.waitForTimeout(2000);

        // Wait for ExoUIRender to complete
        await page.waitForSelector('.exocortex-layout-container', { timeout: 10000 });
        ...
        const createTaskButton = await page.locator('button:has-text("Create Child Task")');
    ...
    test('Button click should create a new task', async () => {
        ...
        const notice = await page.locator('.notice:has-text("Task created successfully")');
    ...
    test('Layout should load from correct folder path', async () => {
        ...
        const layoutPath = path.join(process.cwd(), 'layouts', 'Layout - ems__Project.md');
        ...
        const customLayout = await page.locator('.exocortex-layout-container[data-layout="custom"]');
        await expect(customLayout).toBeVisible();

        // Verify specific blocks from the custom layout
        const projectActionsBlock = await page.locator('.exocortex-block-title:has-text("🚀 Project Actions")');
        await expect(projectActionsBlock).toBeVisible();

        const projectInfoBlock = await page.locator('.exocortex-block-title:has-text("📋 Project Information")');
        await expect(projectInfoBlock).toBeVisible();

        const activeTasksBlock = await page.locator('.exocortex-block-title:has-text("📝 Active Tasks")');
...
`;
        ...
        await page.goto(`obsidian://open?vault=${encodeURIComponent(vaultPath)}&file=test-project.md`);
        await page.waitForTimeout(2000);
        await page.waitForSelector('.exocortex-layout-container', { timeout: 10000 });
...
`;
        ...
        // Open the test task
        await page.goto(`obsidian://open?vault=${encodeURIComponent(vaultPath)}&file=test-task.md`);
        await page.waitForTimeout(2000);
    ...
    test('Layout changes should reflect immediately', async () => {
        ...
        const modifiedLayout = await page.evaluate(async () => {
            const app = (window as any).app;
            const layoutFile = app.vault.getAbstractFileByPath('layouts/Layout - ems__Project.md');
            if (layoutFile) {
...
test.describe('Error Handling and Edge Cases', () => {
    ...
    test('Should handle missing layout file gracefully', async ({ page }) => {
        ...
        try {
            await page.goto(`obsidian://open?vault=${encodeURIComponent('/Users/kitelev/vault-2025')}&file=${encodeURIComponent('01 Inbox/Project - Антифрод-триггеры.md')}`);
            await page.waitForTimeout(2000);
...
`;
        ...
        await page.goto(`obsidian://open?vault=${encodeURIComponent('/Users/kitelev/vault-2025')}&file=malformed-project.md`);
        await page.waitForTimeout(2000);

        // Should handle gracefully without crashing
        const container = await page.locator('.exocortex-layout-container, .exocortex-default-layout, .exocortex-error');
        await expect(container).toBeVisible();
    });

    test('Should handle network/permission errors during task creation', async ({ page }) => {
        await page.goto(`obsidian://open?vault=${encodeURIComponent('/Users/kitelev/vault-2025')}&file=${encodeURIComponent('01 Inbox/Project - Антифрод-триггеры.md')}`);
        await page.waitForTimeout(2000);
        await page.waitForSelector('.exocortex-layout-container', { timeout: 10000 });
        ...
        const errorNotice = await page.locator('.notice:has-text("Error"), .notice:has-text("Failed")');
