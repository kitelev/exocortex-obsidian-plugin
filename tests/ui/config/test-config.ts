...
class TestConfigManager {
  ...
  private createConfig(): TestConfig {
    ...
    return {
      timeouts: {
        modal: parseInt(
          process.env.TEST_MODAL_TIMEOUT || baseTimeouts.modal.toString(),
        ),
        element: parseInt(
          process.env.TEST_ELEMENT_TIMEOUT || baseTimeouts.element.toString(),
        ),
        content: parseInt(
          process.env.TEST_CONTENT_TIMEOUT || baseTimeouts.content.toString(),
        ),
        interaction: parseInt(
          process.env.TEST_INTERACTION_TIMEOUT ||
            baseTimeouts.interaction.toString(),
        ),
      },
      retries: {
        operations: isCI ? 5 : 3,
...
export const TEST_TIMEOUTS = {
  get MODAL() {
    return testConfig.getTimeout("modal");
  },
  get ELEMENT() {
    return testConfig.getTimeout("element");
  },
  get CONTENT() {
    return testConfig.getTimeout("content");
  },
  get INTERACTION() {
    return testConfig.getTimeout("interaction");
  },
};

export const TEST_RETRIES = {
  get OPERATIONS() {
    return testConfig.getRetries("operations");
  },
  get TESTS() {
    return testConfig.getRetries("tests");
  },
};
