...
test.describe('Exocortex Plugin Settings UI Tests', () => {
    ...
    test('Settings tab should be visible in Obsidian settings', async () => {
        ...
        const exocortexPlugin = await page.locator('.installed-plugin').filter({ hasText: 'Exocortex' });
        await expect(exocortexPlugin).toBeVisible({ timeout: 5000 });

        // Click on the settings gear icon for Exocortex
        const settingsGear = await exocortexPlugin.locator('.clickable-icon[aria-label*="Options"]');
        ...
        const folderPathsSection = await page.locator('h2:has-text("Folder Paths")');
        await expect(folderPathsSection).toBeVisible();

        const queryEngineSection = await page.locator('h2:has-text("Query Engine")');
    ...
    test('Layouts folder path setting should be configurable', async () => {
        ...
        const layoutsFolderInput = await page.locator('input[placeholder*="layouts"]').first();
    ...
    test('Debug mode toggle should work', async () => {
        ...
        const debugToggle = await page.locator('.checkbox-container').filter({ hasText: 'Debug mode' });
    ...
    test('Settings should persist after reload', async () => {
        ...
        const templatesFolderInput = await page.locator('input[placeholder*="templates"]').first();
    ...
    test('Reset to defaults should work', async () => {
        ...
        // Find reset button
        const resetButton = await page.locator('button:has-text("Reset to defaults")');
        ...
        // Confirm in dialog
        const confirmButton = await page.locator('.modal-button-container button:has-text("Reset")');
    ...
    test('Performance sliders should show current values', async () => {
        ...
        const maxGraphSizeSlider = await page.locator('input[type="range"]').first();
    ...
    test('Query engine dropdown should have options', async () => {
        ...
        // Get options
        const options = await queryEngineDropdown.locator('option').allTextContents();
...
test.describe('Settings Integration with Plugin Features', () => {
    ...
    test('Layouts folder setting should affect button rendering', async ({ page }) => {
        // Change layouts folder path
        await page.goto(`obsidian://open?vault=${encodeURIComponent('/Users/kitelev/vault-2025')}`);
        ...
        await page.goto(`obsidian://open?vault=${encodeURIComponent('/Users/kitelev/vault-2025')}&file=${encodeURIComponent(projectFilePath)}`);
    ...
    test('Debug mode should enable verbose logging', async ({ page }) => {
        ...
        if (logs.length > 0) {
            expect(logs.some(log => log.includes('debug') || log.includes('Debug'))).toBeTruthy();
        }
    });
});