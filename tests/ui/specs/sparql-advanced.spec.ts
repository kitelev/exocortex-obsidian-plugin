...
# Complex Task`
      ...
      const properties = ['exo__Asset_uid', 'exo__Instance_class', 'exo__Asset_label', 
                         'ems__Task_status', 'ems__Task_priority', 'ems__Task_assignee'];
      
      properties.forEach(prop => {
        const hasProp = rows.some(row => row.some(cell => cell.includes(prop)));
...
\`\`\``
      ...
      const tagRows = rows.filter(row => row.some(cell => cell.includes('tags')));
...
\`\`\``
  ...
  describe('Edge Cases', () => {
    it('should handle empty vault gracefully', async () => {
      ...
      await browser.executeObsidian(({ app }) => {
        app.vault.create('test-empty-vault.md', '```sparql\nSELECT * WHERE { } LIMIT 5\n```');
