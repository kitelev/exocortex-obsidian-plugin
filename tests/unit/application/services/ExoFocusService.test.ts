...
import {
  ExoFocus,
  FocusFilter,
} from "../../../../src/domain/entities/ExoFocus";
...
describe("ExoFocusService", () => {
  ...
  describe("setActiveFocus", () => {
    ...
    it("should set active focus successfully", async () => {
      const createResult = await exoFocusService.createFocus(
        "Test Focus",
        "Description",
        [],
      );
    ...
    it("should deactivate previous focus when setting new one", async () => {
      const focus1Result = await exoFocusService.createFocus(
        "Focus 1",
        "First focus",
        [],
      );
      const focus2Result = await exoFocusService.createFocus(
        "Focus 2",
        "Second focus",
        [],
      );
  ...
  describe("createFocus", () => {
    ...
    it("should create focus with valid parameters", async () => {
      ...
      const result = await exoFocusService.createFocus(
        "Work Focus",
        "Work related items",
        filters,
      );
  ...
  describe("updateFocus", () => {
    ...
    it("should update focus priority successfully", async () => {
      const result = await exoFocusService.updateFocus(focusId, {
        priority: 75,
      });
    ...
    it("should fail when focus does not exist", async () => {
      const result = await exoFocusService.updateFocus("non-existent", {
        priority: 75,
      });
  ...
  describe("filterTriples", () => {
    ...
    beforeEach(async () => {
      ...
      mockTriple1 = {
        ...
        getPredicate: jest
          .fn()
          .mockReturnValue({ toString: () => "predicate1" }),
      ...
      mockTriple2 = {
        ...
        getPredicate: jest
          .fn()
          .mockReturnValue({ toString: () => "predicate2" }),
  ...
  describe("save functionality", () => {
    ...
    it("should save both config and active focus files", async () => {
      ...
      const configCall = writeCalls.find(
        (call) => call[0] === ".exocortex/focus-configs.json",
      );
      const activeCall = writeCalls.find(
        (call) => call[0] === ".exocortex/focus.json",
      );
    ...
    it("should skip saving when no adapter available", async () => {
      const appWithoutAdapter = { vault: {} } as any;
      const serviceWithoutAdapter = new ExoFocusService(
        appWithoutAdapter,
        mockGraph,
      );
      ...
      try {
        ...
        throw new Error(
          "createFocus should not throw when no adapter available",
        );
      }
    });
  });
  ...
  describe("edge cases and error handling", () => {
    ...
    it("should handle very large filter arrays", async () => {
      const largeFilters: FocusFilter[] = Array.from(
        { length: 1000 },
        (_, i) => ({
          type: "tag",
          operator: "includes",
