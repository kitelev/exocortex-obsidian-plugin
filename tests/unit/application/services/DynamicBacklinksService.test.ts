...
describe("DynamicBacklinksService", () => {
  ...
  describe("discoverPropertyBasedBacklinks", () => {
      ...
      mockApp.metadataCache.getFileCache = jest
        ...
        .mockImplementation((file: TFile) => {
          switch (file.path) {
            case "file1.md":
              return {
                frontmatter: { parent: "target", exo__Instance_class: "Task" },
              };
            case "file2.md":
              return {
                frontmatter: {
    ...
    it("should exclude specified properties", async () => {
      ...
      expect(
        backlinks.find((b) => b.propertyName === "exo__Asset_id"),
      ).toBeUndefined();
    });
      ...
      mockApp.metadataCache.getFileCache = jest
        ...
        .mockImplementation((file: TFile) => {
          switch (file.path) {
            ...
            case "note.md":
              return {
                frontmatter: { parent: "target", exo__Instance_class: "Note" },
              };
            default:
              return { frontmatter: {} };
          }
    ...
    it("should handle various reference formats", async () => {
      ...
      mockApp.metadataCache.getFileCache = jest
        ...
        .mockImplementation((file: TFile) => {
          switch (file.path) {
            case "file1.md":
              return { frontmatter: { ref: "target" } };
            case "file2.md":
              return { frontmatter: { ref: "[[target]]" } };
            case "file3.md":
              return { frontmatter: { ref: "[[target|display]]" } };
            case "file4.md":
              return { frontmatter: { ref: ["other", "target"] } };
            default:
              return { frontmatter: {} };
          }
        });
    ...
    it("should handle exocortex Area/Effort/Project structure with UUID matching", async () => {
      ...
      const effortFile = createMockFile(
        "03 Knowledge/toos/Project - Test Project.md",
        {
      ...
      mockApp.metadataCache.getFirstLinkpathDest = jest
        .fn()
        .mockImplementation((linkText: string, sourcePath: string) => {
          if (linkText === "Area - My") {
      ...
      expect(referencingPaths).toContain(
        "03 Knowledge/toos/Project - Test Project.md",
      );
      expect(referencingPaths).toContain(
        "03 Knowledge/toos/Task - Test Task.md",
      );
    });
    ...
    it("should handle path-based link resolution for Area references", async () => {
      ...
      expect(backlinks[0].propertyName).toBe("ems__Effort_area");
      expect(backlinks[0].referencingFiles).toHaveLength(1);
      expect(backlinks[0].referencingFiles[0].path).toBe(
        "Projects/Important Project.md",
      );
    });
    ...
    it("should handle realistic exocortex structure with multiple property types", async () => {
      ...
      const effort1 = createMockFile(
        "03 Knowledge/toos/Project - Business Trip.md",
        {
          exo__Instance_class: ["[[ems__Project]]"],
          ems__Effort_area: "[[Area - My]]",
        },
      ...
      const effort2 = createMockFile(
        "03 Knowledge/kitelev/Task - Review Code.md",
        {
          exo__Instance_class: ["[[ems__Task]]"],
          ems__Effort_area: "[[Area - My]]",
        },
      ...
      const effort3 = createMockFile(
        "03 Knowledge/kitelev/Project - Plugin Development.md",
        {
          exo__Instance_class: ["[[ems__Project]]"],
      ...
      expect(effortAreaFiles).toContain(
        "03 Knowledge/toos/Project - Business Trip.md",
      );
      expect(effortAreaFiles).toContain(
        "03 Knowledge/kitelev/Task - Review Code.md",
      ...
      expect(backlinks[1].referencingFiles[0].path).toBe(
        "03 Knowledge/kitelev/Project - Plugin Development.md",
      );
    });
    ...
    it("should skip target file itself", async () => {
      ...
      expect(backlinks).toHaveLength(1);
      expect(
        backlinks[0].referencingFiles.some((f) => f.path === "target.md"),
      ).toBe(false);
    });
