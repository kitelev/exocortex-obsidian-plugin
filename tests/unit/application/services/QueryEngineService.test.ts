...
describe("QueryEngineService", () => {
  ...
  beforeEach(() => {
    ...
    mockQueryEngineFactory.createQueryEngine.mockResolvedValue(
      Result.ok<IQueryEngine>(mockQueryEngine),
    );
  });
  ...
  describe("Query Execution", () => {
    ...
    beforeEach(() => {
      mockQueryEngine.executeQuery.mockResolvedValue(
        Result.ok<QueryResult>(mockQueryResult),
      );
    });
    ...
    it("should execute query with preferred engine", async () => {
      ...
      expect(mockQueryEngineFactory.createQueryEngine).toHaveBeenCalledWith(
        "dataview",
      );
    ...
    it("should not cache failed results", async () => {
      ...
      mockQueryEngine.executeQuery.mockResolvedValue(
        Result.fail<QueryResult>("Query failed"),
      );
  ...
  describe("Engine Fallback", () => {
    it("should fallback to alternative engine on failure", async () => {
      ...
      mockQueryEngineFactory.createQueryEngine
        .mockResolvedValueOnce(
          Result.fail<IQueryEngine>("Primary engine failed"),
        )
        .mockResolvedValueOnce(Result.ok<IQueryEngine>(mockFallbackEngine));

      mockFallbackEngine.executeQuery.mockResolvedValue(
    ...
    it("should auto-detect available engine when configured engines fail", async () => {
      ...
      mockAutoEngine.executeQuery.mockResolvedValue(
        Result.ok<QueryResult>({
          success: true,
          data: [],
          metadata: {},
    ...
    it("should fail when no engines are available", async () => {
      mockQueryEngineFactory.createQueryEngine.mockResolvedValue(
        Result.fail<IQueryEngine>("No engines available"),
      );
  ...
  describe("Engine Reuse", () => {
    ...
    it("should create new engine if type changed", async () => {
      ...
      expect(mockQueryEngineFactory.createQueryEngine).toHaveBeenNthCalledWith(
        1,
        "dataview",
      );
      expect(mockQueryEngineFactory.createQueryEngine).toHaveBeenNthCalledWith(
  ...
  describe("Render Query", () => {
    it("should render query to container", async () => {
      ...
      expect(result.isSuccess).toBe(true);
      expect(mockQueryEngine.renderQuery).toHaveBeenCalledWith(
        container,
        query,
        undefined,
      );
    ...
    it("should show error in container when engine fails", async () => {
      ...
      mockQueryEngineFactory.createQueryEngine.mockResolvedValue(
        Result.fail<IQueryEngine>("Engine failed"),
      );
      ...
      expect(container.textContent).toContain(
        "Query Engine Error: No query engines available",
      );
    });
  });
  ...
  describe("Get Pages", () => {
    ...
    it("should handle engine errors for getPages", async () => {
      mockQueryEngineFactory.createQueryEngine.mockResolvedValue(
        Result.fail<IQueryEngine>("Engine not available"),
      );
  ...
  describe("Get Page Metadata", () => {
    it("should get page metadata from engine", async () => {
      ...
      mockQueryEngine.getPageMetadata.mockResolvedValue(
        Result.ok<Record<string, any>>(metadata),
      );
  ...
  describe("Query Validation", () => {
    it("should validate query syntax", async () => {
      ...
      expect(mockQueryEngine.validateQuery).toHaveBeenCalledWith(
        'LIST FROM "folder"',
      );
    });
    ...
    it("should handle validation with specific engine type", async () => {
      ...
      mockQueryEngineFactory.createQueryEngine.mockResolvedValue(
        Result.ok<IQueryEngine>(mockSparqlEngine),
      );
  ...
  describe("Engine Information", () => {
    ...
    it("should check engine availability", () => {
      ...
      expect(mockQueryEngineFactory.isEngineAvailable).toHaveBeenCalledWith(
        "dataview",
      );
    });
  });
