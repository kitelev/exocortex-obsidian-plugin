...
describe("ChildrenEffortsBlockRenderer - Matching Logic", () => {
  ...
  describe("Full rendering with real-world data", () => {
    it("should show children efforts for Антифрод-триггеры project", async () => {
      ...
      // Mock backlinks
      (mockApp.metadataCache as any).getBacklinksForFile = jest
        .fn()
        .mockReturnValue({
          data: new Map([
            [
              "01 Inbox/Project - Взять в работу эпик по антифрод-триггерам.md",
              1,
            ],
          ]),
        });
      ...
      expect(container.innerHTML).toContain(
        "Взять в работу эпик по антифрод-триггерам",
      );
