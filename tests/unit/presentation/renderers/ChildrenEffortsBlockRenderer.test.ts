...
describe('ChildrenEffortsBlockRenderer', () => {
    ...
    describe('render', () => {
        ...
        it('should render children efforts as a table with Asset Name and Status columns', async () => {
            ...
            const table = mockContainer.querySelector('.exocortex-children-efforts-table');
        ...
        it('should extract and format effort status correctly', async () => {
            ...
            const table = mockContainer.querySelector('.exocortex-children-efforts-table');
            const statusCells = table.querySelectorAll('.exocortex-table-cell-status span');
            ...
            expect(statusCells[0].classList.contains('exocortex-status-known')).toBeTruthy();
            expect(statusCells[1].classList.contains('exocortex-status-known')).toBeTruthy();
            expect(statusCells[2].classList.contains('exocortex-status-unknown')).toBeTruthy();
        });
        ...
        it('should handle array format for ems__Effort_status', async () => {
            ...
            mockApp.metadataCache.getFileCache.mockReturnValue({
                frontmatter: {
                    ...
                    'ems__Effort_status': ['[[ems__EffortStatusInProgress]]', 'secondary-status']
                }
            });

            await renderer.render(mockContainer, config, mockFile, null);

            const statusCell = mockContainer.querySelector('.exocortex-table-cell-status span');
        ...
        it('should group by class when groupByClass is enabled and render each group as table', async () => {
            ...
            const tables = mockContainer.querySelectorAll('.exocortex-children-efforts-table');
            ...
            const statusCells = mockContainer.querySelectorAll('.exocortex-table-cell-status');
        ...
        it('should display dash for missing parent when showParentPath is enabled', async () => {
            ...
            const parentCell = mockContainer.querySelector('.exocortex-table-cell-parent');
            expect(parentCell.textContent).toBe('test-file');
        });
    ...
    describe('extractEffortStatus', () => {
        ...
        beforeEach(() => {
            ...
            extractEffortStatusMethod = (renderer as any).extractEffortStatus.bind(renderer);
        });
        ...
        it('should handle array format and use first element', () => {
            const frontmatter = { 'ems__Effort_status': ['[[ems__EffortStatusInProgress]]', 'secondary'] };
            const result = extractEffortStatusMethod(frontmatter);
