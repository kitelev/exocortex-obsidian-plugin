...
describe("GraphVisualizationProcessor - Export Functionality", () => {
  ...
  describe("PNG Export", () => {
    it("should create PNG export dropdown with resolution options", async () => {
      ...
      const createDropdown = (processor as any).createPNGExportDropdown.bind(
        processor,
      );
    ...
    it("should export PNG with standard resolution", async () => {
      ...
      // Call export method
      await (processor as any).exportAsPNG(
        container,
        data,
        config,
        1,
    ...
    it("should handle PNG export errors gracefully", async () => {
      ...
      await expect(
        (processor as any).exportAsPNG(container, data, config, 1, "test.png"),
    ...
    it("should export SVG successfully", () => {
      ...
      expect(console.log).toHaveBeenCalledWith(
        expect.stringContaining(
          "Test environment: Would download knowledge-graph.svg",
        ),
      );
  ...
  describe("Canvas Rendering", () => {
    it("should render graph data to canvas", async () => {
      ...
      await (processor as any).renderGraphToCanvas(
        mockContext,
        data,
        config,
        800,
    ...
    it("should render without labels when disabled", async () => {
      ...
      const fillTextCalls = (
        mockContext.fillText as jest.Mock
      ).mock.calls.filter((call) => call[0] === "Node 1");
  ...
  describe("File Download", () => {
    it("should download blob with correct filename", () => {
      ...
      expect(console.log).toHaveBeenCalledWith(
        expect.stringContaining(
          "Test environment: Would download test-file.txt (4 bytes)",
        ),
      );
