...
import { SPARQLEngine, SelectResult, ConstructResult } from '../../../../src/application/SPARQLEngine';
...
describe('SPARQLProcessor', () => {
  ...
  beforeEach(() => {
    ...
    // Mock services
    mockFocusService = new ExoFocusService(mockApp, mockGraph) as jest.Mocked<ExoFocusService>;
    ...
    // Setup service mocks
    (RDFService as jest.MockedClass<typeof RDFService>).mockImplementation(() => mockRDFService);
    (SPARQLSanitizer as jest.MockedClass<typeof SPARQLSanitizer>).mockImplementation(() => mockSanitizer);
  ...
  describe('Initialization', () => {
    ...
    test('should initialize with cache configuration', () => {
      ...
      const processorWithCache = new SPARQLProcessor(mockPlugin, mockGraph, mockFocusService, cacheConfig);
  ...
  describe('processCodeBlock', () => {
    test('should process simple SELECT query successfully', async () => {
      ...
      expect(sparqlContainer?.querySelector('h3')?.textContent).toBe('SPARQL Query Results');
    ...
    test('should handle empty results', async () => {
      ...
      expect(container.querySelector('.sparql-empty-result')?.textContent).toBe('No results found');
    });
    ...
    test('should handle cached results', async () => {
      ...
      expect(container.querySelector('.sparql-cache-indicator')?.textContent).toContain('Cached Result');
    });
    ...
    test('should display loading indicator', async () => {
      ...
      expect(container.querySelector('.sparql-loading')?.textContent).toBe('Executing query...');
    ...
    test('should handle query execution errors', async () => {
      ...
      expect(container.querySelector('.sparql-error')?.textContent).toContain('Query validation failed');
    });
  ...
  describe('executeQuery', () => {
    ...
    test('should handle query sanitization failures', async () => {
      ...
      await expect(processor.executeQuery('DROP TABLE users;')).rejects.toThrow('Query validation failed');
    });
    ...
    test('should reject unsupported query types', async () => {
      ...
      await expect(processor.executeQuery('DELETE WHERE { ?s ?p ?o }')).rejects.toThrow('Only SELECT and CONSTRUCT queries are currently supported');
    });
    ...
    test('should handle CONSTRUCT query with duplicate prevention', async () => {
      ...
      const result = await processor.executeQuery('CONSTRUCT { ?s ?p ?o } WHERE { ?s ?p ?o }');
  ...
  describe('Export Controls', () => {
    ...
    test('should handle export button clicks', async () => {
      ...
      const controls = processor['createExportControls'](results, query);
      const turtleButton = controls.querySelector('.sparql-export-button') as HTMLButtonElement;
...
function createMockContext(sourcePath: string = 'test.md'): MarkdownPostProcessorContext {
  return {
    sourcePath,
    frontmatter: {},
    addChild: jest.fn(),
