...
describe("IndexedGraph Performance Optimization Validation", () => {
  ...
  describe("ISO/IEC 25010 Time Behavior Compliance", () => {
    it("should achieve sub-100ms query response for 10k+ triples", () => {
      ...
      console.log(
        `Query performance: ${queryTime.toFixed(2)}ms for ${queryPatterns.length} patterns`,
      );
      console.log(
        `Average per query: ${(queryTime / queryPatterns.length).toFixed(2)}ms`,
      );
    ...
    it("should demonstrate optimized property hierarchy traversal", () => {
      ...
      const startTime = performance.now();
      const hierarchyResults = graph.queryPropertyHierarchy(
        "http://example.org/property_4_5",
        "broader",
      );
      const traversalTime = performance.now() - startTime;

      console.log(
        `Hierarchy traversal: ${traversalTime.toFixed(2)}ms, found ${hierarchyResults.length} related properties`,
      );
      ...
      const cachedResults = graph.queryPropertyHierarchy(
        "http://example.org/property_4_5",
        "broader",
      );
  ...
  describe("Resource Utilization Optimization", () => {
    it("should maintain efficient memory usage during bulk operations", () => {
      ...
      console.log(
        `Memory usage: ${(memoryGrowth / 1024 / 1024).toFixed(2)}MB for ${TOTAL_TRIPLES} triples`,
      );
      console.log(
        `Memory per triple: ${(memoryGrowth / TOTAL_TRIPLES).toFixed(0)} bytes`,
      );
      console.log(
        `Index sizes: SPO=${stats.indexSizes.spo}, POS=${stats.indexSizes.pos}, OSP=${stats.indexSizes.osp}`,
      );
    ...
    it("should demonstrate cache effectiveness and hit rates", () => {
      ...
      console.log(
        `Cached query time: ${cachedQueryTime.toFixed(2)}ms for ${queries.length} queries`,
      );
  ...
  describe("Capacity and Scalability", () => {
    it("should handle concurrent query load efficiently", async () => {
      ...
      const patterns = Array.from({ length: 100 }, (_, i) => ({
        s:
          Math.random() < 0.3
            ? `http://example.org/subject${i * 20}`
            : undefined,
        p:
          Math.random() < 0.7
            ? `http://example.org/predicate${i % 50}`
            : undefined,
        o: undefined,
      }));
      ...
      console.log(
        `Parallel queries: ${patterns.length} patterns in ${parallelTime.toFixed(2)}ms`,
      );
      console.log(
        `Throughput: ${(patterns.length / (parallelTime / 1000)).toFixed(0)} queries/sec`,
      );
    ...
    it("should pass ISO/IEC 25010 performance compliance validation", () => {
      ...
      console.log(
        `Time Behavior: ${compliance.timeBehavior.compliant ? "✓" : "✗"} (${compliance.timeBehavior.responseTime.toFixed(2)}ms avg)`,
      );
      console.log(
        `Resource Utilization: ${compliance.resourceUtilization.compliant ? "✓" : "✗"} (${compliance.resourceUtilization.memoryUsage.toFixed(1)}% memory, ${compliance.resourceUtilization.cpuEfficiency.toFixed(0)} ops/sec)`,
      );
      console.log(
        `Capacity: ${compliance.capacity.compliant ? "✓" : "✗"} (${compliance.capacity.currentTriples}/${compliance.capacity.maxTriples} triples)`,
      );
  ...
  describe("Semantic Query Optimization", () => {
    it("should optimize exo__Property semantic searches", () => {
      ...
      console.log(
        `Semantic query performance: ${semanticTime.toFixed(2)}ms for ${semanticPatterns.length} patterns`,
      );
      console.log(
        `Results: ${semanticResults.map((r) => r.length).join(", ")} matches`,
      );
    ...
    it("should demonstrate batch semantic processing efficiency", () => {
      ...
      for (let i = 0; i < 500; i++) {
        ...
        graph.add(
          new Triple(
            new IRI(propId),
            new IRI("rdfs:domain"),
            new IRI(`http://example.org/class${i % 15}`),
          ),
        );
        graph.add(
          new Triple(
      ...
      console.log(
        `Batch semantic processing: ${batchPatterns.length} patterns in ${batchTime.toFixed(2)}ms`,
      );
      console.log(
        `Processing rate: ${metrics.batchProcessingRate.toFixed(0)} patterns/sec`,
      );
  ...
  describe("Memory Management and Optimization", () => {
    it("should demonstrate automatic performance optimization", () => {
      ...
      for (let i = 0; i < 100; i++) {
        graph.query(
          `http://example.org/subject${i * 30}`,
          `http://example.org/predicate${i % 100}`,
        );
      }
      ...
      console.log(
        `- Average query time: ${metrics.averageQueryTime.toFixed(3)}ms`,
      );
      console.log(
        `- Cache hit rate: ${(metrics.cacheHitRate * 100).toFixed(1)}%`,
      );
      ...
      console.log(
        `- Index fragmentation: ${(metrics.indexFragmentation * 100).toFixed(1)}%`,
      );
