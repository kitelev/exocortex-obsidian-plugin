...
import { IRI, Literal, EXO, EMS } from '../../../../src/domain/semantic/core/Triple';
...
describe('KnowledgeObject', () => {
  describe('creation', () => {
    ...
    it('should create with initial properties', () => {
      ...
      expect(ko.getProperty(new IRI('https://example.org/label'))).toBe('Test Task');
  ...
  describe('graph operations', () => {
    it('should export to graph', () => {
      ...
      const typeTriples = graph.match(subject, 
    ...
    it('should reconstruct from graph', () => {
      ...
      const result = KnowledgeObject.fromGraph(original.uuid, graph, original.content);
      ...
      expect(reconstructed.getProperty(new IRI('https://example.org/label'))).toBe('Test Task');
    });
  });
  ...
  describe('cloning', () => {
    it('should create independent clone', () => {
      ...
      expect(clone.getProperty(new IRI('https://example.org/label'))).toBe('Original');
      
      // Modify clone
      clone.setProperty(new IRI('https://example.org/label'), 'Modified');
      
      // Original should be unchanged
      expect(original.getProperty(new IRI('https://example.org/label'))).toBe('Original');
      expect(clone.getProperty(new IRI('https://example.org/label'))).toBe('Modified');
    });
  });
  ...
  describe('getAllProperties', () => {
    it('should return all properties as a map', () => {
      ...
      const hasUuid = Array.from(allProps.keys()).some(k => k.toString() === EXO.uuid.toString());
      const hasCreatedAt = Array.from(allProps.keys()).some(k => k.toString() === EXO.createdAt.toString());
      const hasUpdatedAt = Array.from(allProps.keys()).some(k => k.toString() === EXO.updatedAt.toString());
      ...
      const labelValues = Array.from(allProps.entries())
        .find(([k, v]) => k.toString() === 'https://example.org/label')?.[1];
      const priorityValues = Array.from(allProps.entries())
        .find(([k, v]) => k.toString() === 'https://example.org/priority')?.[1];
      const tagValues = Array.from(allProps.entries())
        .find(([k, v]) => k.toString() === 'https://example.org/tags')?.[1];
