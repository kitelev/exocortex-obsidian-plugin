...
describe("TaskStatus", () => {
  ...
  describe("static factory methods", () => {
    it("should create all status types", () => {
      ...
      expect(TaskStatus.inProgress().getStatus()).toBe(
  ...
  describe("markdown checkbox conversion", () => {
    ...
    it("should parse from markdown checkboxes", () => {
      expect(
        TaskStatus.fromMarkdownCheckbox("- [ ]").getValue().getStatus(),
      ).toBe(TaskStatusType.TODO);
      expect(
        TaskStatus.fromMarkdownCheckbox("- [/]").getValue().getStatus(),
      ).toBe(TaskStatusType.IN_PROGRESS);
      expect(
        TaskStatus.fromMarkdownCheckbox("- [-]").getValue().getStatus(),
      ).toBe(TaskStatusType.WAITING);
      expect(
        TaskStatus.fromMarkdownCheckbox("- [x]").getValue().getStatus(),
      ).toBe(TaskStatusType.DONE);
      expect(
        TaskStatus.fromMarkdownCheckbox("- [X]").getValue().getStatus(),
      ).toBe(TaskStatusType.DONE);
      expect(
        TaskStatus.fromMarkdownCheckbox("- [~]").getValue().getStatus(),
      ).toBe(TaskStatusType.CANCELLED);
    });
