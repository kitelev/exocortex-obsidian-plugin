...
describe("ButtonGroupsBuilder", () => {
  ...
  describe("build", () => {
    it("should return only maintenance buttons for non-creation asset classes", async () => {
      ...
      mockMetadataExtractor.extractInstanceClass.mockReturnValue("[[unsupported]]");
    ...
    it("should build creation buttons group for Area", async () => {
      const mockFile = { path: "test.md", parent: { path: "Areas" }, basename: "TestArea" } as TFile;
      const metadata = { exo__Instance_class: "[[ems__Area]]" };

      mockMetadataExtractor.extractMetadata.mockReturnValue(metadata);
      mockMetadataExtractor.extractInstanceClass.mockReturnValue("[[ems__Area]]");
      mockMetadataExtractor.extractStatus.mockReturnValue(null);
      mockMetadataExtractor.extractIsArchived.mockReturnValue(false);
      mockFolderRepairService.getExpectedFolder.mockResolvedValue("Areas/TestArea");

      const groups = await builder.build(mockFile);

      const creationGroup = groups.find(g => g.id === "creation");
      expect(creationGroup).toBeDefined();
      expect(creationGroup?.buttons.some(b => b.id === "create-task")).toBe(true);
      expect(creationGroup?.buttons.some(b => b.id === "create-project")).toBe(true);
      expect(creationGroup?.buttons.some(b => b.id === "create-area")).toBe(true);
    });
    ...
    it("should build status buttons group for Task with Draft status", async () => {
      ...
      mockMetadataExtractor.extractStatus.mockReturnValue("[[ems__EffortStatusDraft]]");
      mockMetadataExtractor.extractIsArchived.mockReturnValue(false);
      ...
      expect(statusGroup?.buttons.some(b => b.id === "move-to-backlog" && b.visible)).toBe(true);
    });
    ...
    it("should build planning buttons group for Task with Backlog status", async () => {
      ...
      mockMetadataExtractor.extractMetadata.mockReturnValue(metadata);
      mockMetadataExtractor.extractInstanceClass.mockReturnValue("[[ems__Task]]");
      mockMetadataExtractor.extractStatus.mockReturnValue("[[ems__EffortStatusBacklog]]");
      mockMetadataExtractor.extractIsArchived.mockReturnValue(false);
      mockFolderRepairService.getExpectedFolder.mockResolvedValue("Tasks");
      ...
      expect(planningGroup?.buttons.some(b => b.id === "plan-on-today" && b.visible)).toBe(true);
      expect(planningGroup?.buttons.some(b => b.id === "vote-on-effort" && b.visible)).toBe(true);
    });
    ...
    it("should build maintenance buttons group", async () => {
      ...
      expect(maintenanceGroup?.buttons.some(b => b.id === "trash")).toBe(true);
      expect(maintenanceGroup?.buttons.some(b => b.id === "clean-properties")).toBe(true);
    });

    it("should show repair folder button when folder mismatch", async () => {
      const mockFile = { path: "wrong/test.md", parent: { path: "wrong" }, basename: "TestTask" } as TFile;
      const metadata = {
        exo__Instance_class: "[[ems__Task]]",
        ems__Effort_status: "[[ems__EffortStatusBacklog]]"
      };
      ...
      const maintenanceGroup = groups.find(g => g.id === "maintenance");
      const repairButton = maintenanceGroup?.buttons.find(b => b.id === "repair-folder");
      expect(repairButton?.visible).toBe(true);
    });

    it("should show rename to UID button when filename != UID", async () => {
      const mockFile = { path: "test.md", parent: { path: "Tasks" }, basename: "wrong-name" } as TFile;
      const metadata = {
        exo__Instance_class: "[[ems__Task]]",
      ...
      const maintenanceGroup = groups.find(g => g.id === "maintenance");
      const renameButton = maintenanceGroup?.buttons.find(b => b.id === "rename-to-uid");
    ...
    it("should show vote count in button label when votes exist", async () => {
      ...
      const planningGroup = groups.find(g => g.id === "planning");
      const voteButton = planningGroup?.buttons.find(b => b.id === "vote-on-effort");
    ...
    it("should show 'Set Active Focus' when no active focus set", async () => {
      ...
      const planningGroup = groups.find(g => g.id === "planning");
      const focusButton = planningGroup?.buttons.find(b => b.id === "set-active-focus");
    ...
    it("should handle array of instance classes", async () => {
      ...
      mockMetadataExtractor.extractMetadata.mockReturnValue(metadata);
      mockMetadataExtractor.extractInstanceClass.mockReturnValue(["[[ems__Task]]", "[[ems__Meeting]]"]);
      mockMetadataExtractor.extractStatus.mockReturnValue("[[ems__EffortStatusBacklog]]");
      mockMetadataExtractor.extractIsArchived.mockReturnValue(false);
      mockFolderRepairService.getExpectedFolder.mockResolvedValue("Tasks");
    ...
    it("should show rollback button for Done status", async () => {
      ...
      mockMetadataExtractor.extractMetadata.mockReturnValue(metadata);
      mockMetadataExtractor.extractInstanceClass.mockReturnValue("[[ems__Task]]");
      mockMetadataExtractor.extractStatus.mockReturnValue("[[ems__EffortStatusDone]]");
      mockMetadataExtractor.extractIsArchived.mockReturnValue(false);
      mockFolderRepairService.getExpectedFolder.mockResolvedValue("Tasks");
      ...
      const statusGroup = groups.find(g => g.id === "status");
      const rollbackButton = statusGroup?.buttons.find(b => b.id === "rollback-status");
      expect(rollbackButton?.visible).toBe(true);
    });

    it("should handle Concept class", async () => {
      const mockFile = { path: "test.md", parent: { path: "Concepts" }, basename: "TestConcept" } as TFile;
      const metadata = { exo__Instance_class: "[[ims__Concept]]" };

      mockMetadataExtractor.extractMetadata.mockReturnValue(metadata);
      mockMetadataExtractor.extractInstanceClass.mockReturnValue("[[ims__Concept]]");
      mockMetadataExtractor.extractStatus.mockReturnValue(null);
      mockMetadataExtractor.extractIsArchived.mockReturnValue(false);
      ...
      const creationGroup = groups.find(g => g.id === "creation");
      const narrowerButton = creationGroup?.buttons.find(b => b.id === "create-narrower-concept");
      expect(narrowerButton?.visible).toBe(true);
    });

    it("should handle TaskPrototype", async () => {
      const mockFile = { path: "test.md", parent: { path: "TaskPrototypes" }, basename: "TestPrototype" } as TFile;
      const metadata = { exo__Instance_class: "[[ems__TaskPrototype]]" };

      mockMetadataExtractor.extractMetadata.mockReturnValue(metadata);
      mockMetadataExtractor.extractInstanceClass.mockReturnValue("[[ems__TaskPrototype]]");
      mockMetadataExtractor.extractStatus.mockReturnValue(null);
      mockMetadataExtractor.extractIsArchived.mockReturnValue(false);
      mockFolderRepairService.getExpectedFolder.mockResolvedValue("TaskPrototypes");

      const groups = await builder.build(mockFile);

      const creationGroup = groups.find(g => g.id === "creation");
      const instanceButton = creationGroup?.buttons.find(b => b.id === "create-instance");
      expect(instanceButton?.visible).toBe(true);
    });

    it("should handle Initiative", async () => {
      const mockFile = { path: "test.md", parent: { path: "Initiatives" }, basename: "TestInitiative" } as TFile;
      const metadata = { exo__Instance_class: "[[ems__Initiative]]" };

      mockMetadataExtractor.extractMetadata.mockReturnValue(metadata);
      mockMetadataExtractor.extractInstanceClass.mockReturnValue("[[ems__Initiative]]");
      mockMetadataExtractor.extractStatus.mockReturnValue(null);
      mockMetadataExtractor.extractIsArchived.mockReturnValue(false);
      mockFolderRepairService.getExpectedFolder.mockResolvedValue("Initiatives");

      const groups = await builder.build(mockFile);

      const creationGroup = groups.find(g => g.id === "creation");
      const projectButton = creationGroup?.buttons.find(b => b.id === "create-project");
    ...
    it("should not show copy label to aliases when no label", async () => {
      ...
      const maintenanceGroup = groups.find(g => g.id === "maintenance");
      const copyButton = maintenanceGroup?.buttons.find(b => b.id === "copy-label-to-aliases");
    ...
    it("should show archive button for Done tasks", async () => {
      ...
      const maintenanceGroup = groups.find(g => g.id === "maintenance");
      const archiveButton = maintenanceGroup?.buttons.find(b => b.id === "archive");
      expect(archiveButton?.visible).toBe(true);
    });

    it("should handle Project with ToDo status", async () => {
      const mockFile = { path: "test.md", parent: { path: "Projects" }, basename: "TestProject" } as TFile;
      const metadata = {
      ...
      mockMetadataExtractor.extractMetadata.mockReturnValue(metadata);
      mockMetadataExtractor.extractInstanceClass.mockReturnValue("[[ems__Project]]");
      mockMetadataExtractor.extractStatus.mockReturnValue("[[ems__EffortStatusToDo]]");
      mockMetadataExtractor.extractIsArchived.mockReturnValue(false);
      ...
      const statusGroup = groups.find(g => g.id === "status");
      const startButton = statusGroup?.buttons.find(b => b.id === "start-effort");
    ...
    it("should handle Task with Doing status", async () => {
      ...
      mockMetadataExtractor.extractMetadata.mockReturnValue(metadata);
      mockMetadataExtractor.extractInstanceClass.mockReturnValue("[[ems__Task]]");
      mockMetadataExtractor.extractStatus.mockReturnValue("[[ems__EffortStatusDoing]]");
      mockMetadataExtractor.extractIsArchived.mockReturnValue(false);
      mockFolderRepairService.getExpectedFolder.mockResolvedValue("Tasks");
