...
describe("CreateAssetModal Dynamic Property Update", () => {
    ...
    app = {
      ...
      metadataCache: {
        getFileCache: jest.fn((file: TFile) => {
          ...
          if (
            file.basename === "exo__Asset" ||
            file.basename === "exo__Person"
          ) {
            return {
              frontmatter: {
          ...
          if (
            file.basename === "exo__firstName" ||
            file.basename === "exo__lastName"
          ) {
            return {
              frontmatter: {
                exo__Instance_class: "[[exo__Property]]",
  ...
  describe("Dynamic Property Updates", () => {
    it("should clear previous properties when class changes", async () => {
      ...
      const hasFirstName = personProperties.some(
        (p: any) => p.label === "First Name",
      );
    ...
    it("should handle array domains correctly", async () => {
      ...
      const properties = modal["properties"];
      const hasMultiDomain = properties.some(
        (p: any) => p.label === "Multi Domain Property",
      );
