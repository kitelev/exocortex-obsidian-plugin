...
This is test content with [[links]] and metadata.`),
  ...
  describe("Plugin Lifecycle", () => {
    ...
    test("should register ribbon icon", async () => {
      ...
      expect(ribbonSpy).toHaveBeenCalledWith(
        "plus-circle",
        "Create ExoAsset",
        expect.any(Function),
      );
  ...
  describe("SPARQL Cache Management", () => {
    ...
    test("should provide cache statistics through command", async () => {
      ...
      const cacheStatsCommand = commands.find(
        (call) => call[0].id === "view-sparql-cache-stats",
      );
    ...
    test("should provide cache clearing functionality", async () => {
      const commands = plugin["addCommand"].mock.calls;
      const clearCacheCommand = commands.find(
        (call) => call[0].id === "clear-sparql-cache",
      );
...
# Test Content`;
      ...
      expect(triples.some((t) => t.getPredicate().toString() === "title")).toBe(
        true,
      );
      expect(
        triples.some((t) => t.getPredicate().toString() === "author"),
      ).toBe(true);
    });
...
Just content, no frontmatter.`;
    ...
    test("should update graph when file is modified", async () => {
      ...
      const modifyHandler = eventHandlers.find(
        (call) => call[0] === "modify",
      )[1];
    ...
    test("should handle file creation events", async () => {
      ...
      const eventHandlers = app.vault.on.mock.calls;
      const createHandler = eventHandlers.find(
        (call) => call[0] === "create",
      )[1];
    ...
    test("should handle file deletion events", async () => {
      ...
      const eventHandlers = app.vault.on.mock.calls;
      const deleteHandler = eventHandlers.find(
        (call) => call[0] === "delete",
      )[1];
  ...
  describe("Command Integration", () => {
    ...
    test("should execute create asset command", async () => {
      ...
      expect(createCommand[0].hotkeys).toEqual([
      ...
      const {
        CreateAssetModal,
      } = require("../../src/presentation/modals/CreateAssetModal");
    ...
    test("should execute export graph command", async () => {
      ...
      const exportCommand = commands.find(
        (call) => call[0].id === "export-knowledge-graph",
      );
      ...
      const {
        ExportRDFModal,
      } = require("../../src/presentation/modals/ExportRDFModal");
    ...
    test("should execute import RDF command", async () => {
      ...
      const importCommand = commands.find(
        (call) => call[0].id === "import-rdf-data",
      );
      ...
      const {
        ImportRDFModal,
      } = require("../../src/presentation/modals/ImportRDFModal");
    ...
    test("should execute quick task creation command", async () => {
      const commands = plugin["addCommand"].mock.calls;
      const taskCommand = commands.find(
        (call) => call[0].id === "quick-create-task",
      );

      expect(taskCommand).toBeDefined();
      expect(taskCommand[0].hotkeys).toEqual([
      ...
      const {
        QuickTaskModal,
      } = require("../../src/presentation/modals/QuickTaskModal");
  ...
  describe("Error Handling", () => {
    test("should handle processor registration failures gracefully", async () => {
      plugin.registerMarkdownCodeBlockProcessor = jest
        .fn()
        .mockImplementation(() => {
