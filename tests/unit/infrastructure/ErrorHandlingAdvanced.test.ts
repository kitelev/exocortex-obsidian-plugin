...
describe('Advanced Error Handling and Edge Cases', () => {
    ...
    describe('Repository Error Handling', () => {
        it('should handle complete file system failures gracefully', async () => {
            ...
            const repository = new ObsidianAssetRepository(failingAdapter as any, {} as any);
        ...
        it('should handle data corruption scenarios', async () => {
            ...
            const repository = new ObsidianAssetRepository(corruptedAdapter as any, {} as any);
            ...
            expect(result.getError()).toContain('corrupted') || expect(result.getError()).toContain('invalid');
        });

        it('should handle intermittent failures with retries', async () => {
            const intermittentAdapter = new CorruptedVaultAdapter();
            const repository = new ObsidianAssetRepository(intermittentAdapter as any, {} as any);
        ...
        it('should handle timeout scenarios', async (done) => {
            ...
            const repository = new ObsidianAssetRepository(slowAdapter as any, {} as any);
    ...
    describe('Async Operation Failures', () => {
        it('should handle Promise rejection chains', async () => {
            const repository = new ObsidianTaskRepository(new FailingVaultAdapter() as any, {} as any);
            
            const taskId = TaskId.create('test-task').getValue()!;
            
            // Chain multiple async operations that will fail
            const chainResult = await repository.findById(taskId)
                .then(result => {
                    if (!result.isSuccess) {
                        return Result.fail<Task>('Initial load failed: ' + result.getError());
                    }
        ...
        it('should handle memory pressure during async operations', async () => {
            const repository = new ObsidianOntologyRepository(new CorruptedVaultAdapter() as any);
        ...
        it('should handle event loop blocking scenarios', async () => {
            const startTime = Date.now();
            const repository = new ObsidianClassLayoutRepository(new SlowVaultAdapter(100) as any);
            ...
            for (let i = 0; i < 20; i++) {
                blockingOperations.push(
                    repository.findByClassName(`TestClass${i}`)
                );
            }
        ...
        it('should handle circular dependency resolution failures', async () => {
            const ontologyRepository = new ObsidianOntologyRepository(new CorruptedVaultAdapter() as any);
    ...
    describe('Query Engine Error Handling', () => {
        ...
        it('should handle invalid data type conversions', async () => {
            ...
            for (const query of typeConversionQueries) {
                ...
                if (!result.isSuccess) {
                    expect(result.getError()).toContain('type' || 'conversion' || 'invalid');
                }
            }
        });
    });
    ...
    describe('Resource Management Edge Cases', () => {
        it('should handle resource leaks in error conditions', async () => {
            ...
            const repository = new ObsidianAssetRepository(new FailingVaultAdapter() as any, {} as any);
        ...
        it('should handle cleanup in finally blocks', async () => {
            ...
            const repository = new ResourceTrackingRepository(new FailingVaultAdapter() as any, {} as any);
            const asset = Asset.create({
                name: 'Cleanup Test',
                className: 'TestClass'
            }).getValue()!;
