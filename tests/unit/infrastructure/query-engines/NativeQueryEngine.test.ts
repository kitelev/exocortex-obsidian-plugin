...
describe("NativeQueryEngine", () => {
  ...
  describe("table queries", () => {
    ...
    it("should execute table query with multiple fields", async () => {
      const result = await engine.executeQuery(
        'table file.name, type, priority from "Projects"',
      );
    ...
    it("should execute table query with where clause", async () => {
      const result = await engine.executeQuery(
        'table file.name, type from "Projects" where type = "project"',
      );
...
- [ ] Incomplete task 2
  ...
  describe("getPageMetadata", () => {
    beforeEach(() => {
      ...
      mockApp.metadataCache.getFileCache.mockReturnValue(
        createMockMetadata({ title: "Test File", tags: ["test"] }, [
          "#project",
          "#important",
        ]),
      );
  ...
  describe("renderQuery", () => {
    ...
    it("should render table query", async () => {
      const result = await engine.renderQuery(
        container,
        'table file.name from "Test"',
      );
  ...
  describe("error handling", () => {
    ...
    it("should handle metadata cache errors", async () => {
      ...
      const result = await engine.executeQuery(
        'table file.name, title from "Test"',
      );
