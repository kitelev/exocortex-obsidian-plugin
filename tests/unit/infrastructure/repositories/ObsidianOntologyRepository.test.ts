...
describe("ObsidianOntologyRepository", () => {
  ...
  describe("findByPrefix", () => {
    ...
    beforeEach(() => {
      ...
      jest
        .spyOn(Ontology, "fromFrontmatter")
        .mockImplementation((frontmatter: any) => {
          return {
    ...
    it("should handle fromFrontmatter errors gracefully", async () => {
      ...
      await expect(repository.findByPrefix(mockPrefix)).rejects.toThrow(
        "Invalid frontmatter",
      );
    });
  });
    ...
    it("should return all ontology files", async () => {
      ...
      mockMetadataCache.getFileCache.mockImplementation((file: TFile) => {
        if (file.name.startsWith("!")) {
          return {
            frontmatter: {
              exo__Ontology_prefix: file.name
                .replace("!", "")
                .replace(".md", ""),
  ...
  describe("save", () => {
    ...
    it("should create new ontology file when it does not exist", async () => {
      ...
      expect(mockVault.create).toHaveBeenCalledWith(
        "!test.md",
        expect.stringContaining("exo__Ontology_prefix: test"),
      );
    ...
    it("should handle vault create errors", async () => {
      ...
      await expect(repository.save(mockOntology)).rejects.toThrow(
  ...
  describe("exists", () => {
    ...
    it("should handle vault errors gracefully", async () => {
      ...
      await expect(repository.exists(mockPrefix)).rejects.toThrow(
        "Vault error",
      );
    });
  });
    ...
    it("should handle save with complex nested frontmatter", async () => {
      const complexOntology = {
        ...
        toFrontmatter: () => ({
          exo__Ontology_prefix: "test",
          exo__Ontology_namespace: "http://example.org/test#",
          exo__Ontology_classes: ["Person", "Organization", "Event"],
          exo__Ontology_properties: ["hasName", "hasDescription", "relatedTo"],
  ...
  describe("Integration Scenarios", () => {
    it("should support full CRUD cycle", async () => {
      ...
      jest
        .spyOn(Ontology, "fromFrontmatter")
        .mockImplementation(() => mockOntology);
    ...
    it("should handle batch operations with multiple ontologies", async () => {
      const prefixes = ["batch1", "batch2", "batch3"].map((p) =>
        OntologyPrefix.create(p).getValue(),
      );
