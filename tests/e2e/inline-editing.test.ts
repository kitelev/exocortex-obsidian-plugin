...
test.describe("Inline Property Editing", () => {
  ...
  test('should edit dropdown property without "Asset not found" error', async ({
    page,
  }) => {
    // Open a test asset
    ...
    await expect(
      page.locator('.notice:has-text("Asset not found")'),
    ).not.toBeVisible();

    // Verify success message
    await expect(
      page.locator('.notice:has-text("Property updated")'),
    ).toBeVisible();

    // Verify the value was updated
    await expect(
      page.locator(".exocortex-property-value-readonly"),
    ).toContainText("Bob");
  });
  ...
  test("should validate required fields", async ({ page }) => {
    ...
    await page.click(
      '.exocortex-property-item:has-text("Required Field") .exocortex-property-value-readonly',
    );
    await page.fill("input", "");
    await page.press("Enter");

    // Verify validation error
    await expect(page.locator(".exocortex-property-error")).toContainText(
  ...
  test("should cancel edit on Escape key", async ({ page }) => {
    ...
    const originalValue = await page
      .locator(".exocortex-property-value-readonly")
      .first()
      .textContent();
    await page.click(".exocortex-property-value-readonly").first();
    ...
    await expect(
      page.locator(".exocortex-property-value-readonly").first(),
  ...
  test("should handle empty optional fields", async ({ page }) => {
    ...
    await page.click(
      '.exocortex-property-item:has-text("Optional Field") .exocortex-property-value-readonly',
    );
    await page.fill("input", "");
    await page.press("Enter");
  ...
  test("should handle concurrent edits gracefully", async ({
    ...
    await expect(
      page2.locator('.notice:has-text("Property updated")'),
    ).toBeVisible();
  });
});
