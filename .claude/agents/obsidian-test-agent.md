...
class PlatformMockManager {
  setupMobileMocks(platform: 'ios' | 'android' | 'tablet' | 'desktop'): void {
    ...
    Object.assign(Platform, {
      isMobile: platform === 'mobile' || platform === 'ios' || platform === 'android',
      isMobileApp: platform === 'mobile' || platform === 'ios' || platform === 'android',
...
abstract class ObsidianTestBase {
  ...
  protected setupTestVault(): void {
    ...
    this.vault.__addMockFile('Project.md', '---\ntype: project\n---\n\n# Project');
    this.vault.__addMockFile('Task.md', '---\ntype: task\nstatus: todo\n---\n\n# Task');
  }
...
class MobileTouchTestSuite extends ObsidianTestBase {
  ...
  testTouchGraphController(): void {
    ...
    const pinchStart = MobileTestUtils.createTouchEvent('touchstart', [
      { x: 100, y: 100, id: 0 },
      { x: 200, y: 200, id: 1 }
    ], canvas);
    
    const pinchMove = MobileTestUtils.createTouchEvent('touchmove', [
      { x: 80, y: 80, id: 0 },
      { x: 220, y: 220, id: 1 }
    ], canvas);
...
class VaultTestingPatterns {
  setupVaultWithFileStructure(): void {
    ...
    vault.__addMockFile('Projects/Web Development.md', this.getProjectContent());
    vault.__addMockFile('Tasks/Implement Feature.md', this.getTaskContent());
    vault.__addMockFile('Notes/Daily/2024-01-01.md', this.getDailyNoteContent());
    vault.__addMockFile('Templates/Project Template.md', this.getTemplateContent());
  ...
  testFrontmatterOperations(): void {
    ...
    const status = this.app.metadataCache.getFrontmatterPropertyValue(file, 'status');
...
class ModalTestingPatterns extends ObsidianTestBase {
  ...
  testModalInteraction(): void {
    ...
    // Simulate user input
    const nameInput = modal.contentEl.querySelector('input[name="name"]') as HTMLInputElement;
  ...
  testMobileModalAdaptation(): void {
    ...
    expect(style.getPropertyValue('padding-top')).toContain('safe-area-inset-top');
  }
}
```
...
class SPARQLProcessorTestSuite extends ObsidianTestBase {
  testSPARQLRegistration(): void {
    ...
    plugin.registerMarkdownCodeBlockProcessor('sparql', processor.process.bind(processor));
...
class RepositoryTestingPatterns extends ObsidianTestBase {
  testAssetRepository(): void {
    ...
    const assetFile = this.createMockFileWithFrontmatter('Assets/Project Alpha.md', {
      type: 'project',
      status: 'active',
      created: '2024-01-01'
    });
...
class ObsidianErrorTestingPatterns {
  ...
  testPluginLoadingErrors(): void {
    ...
    jest.spyOn(plugin, 'loadSettings').mockRejectedValue(new Error('Settings load failed'));
...
class ObsidianTestFactory {
  ...
  static createMockProcessorContext(sourcePath: string = 'test.md'): MarkdownPostProcessorContext {
    return {
      sourcePath,
      frontmatter: {},
      addChild: jest.fn(),
